...
/* Iris v4.164 — visible tool-action feedback, automatic reactions to user moments, refined pet, and concise memory UI. */
...
function addSharedAlbumImage(src,caption=""){if(!src)return;normalizeNestData();nestData.sharedAlbum=Array.isArray(nestData.sharedAlbum)?nestData.sharedAlbum:[];nestData.sharedAlbum.unshift({id:crypto.randomUUID(),src:String(src),caption:String(caption||"").slice(0,160),createdAt:Date.now()});nestData.sharedAlbum=nestData.sharedAlbum.slice(0,60);saveNestData();renderSharedAlbum();}
