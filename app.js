...
/* Iris v4.165 — natural tool narration, no automatic Moments replies, deduplicated shared album, and refined companion. */
...
function normalizeNestData(){
  ...
  nestData.sharedAlbum=Array.isArray(nestData.sharedAlbum)?nestData.sharedAlbum:[];
  const albumSeen=new Set();nestData.sharedAlbum=nestData.sharedAlbum.filter(x=>{const src=String(x?.src||"").trim();if(!src||albumSeen.has(src))return false;albumSeen.add(src);return true}).slice(0,60);
  nestData.pet=nestData.pet&&typeof nestData.pet==="object"?nestData.pet:{};
  nestData.pet.name=String(nestData.pet.name||"小家伙");
  nestData.pet.mood=String(nestData.pet.mood||"期待");
  nestData.pet.hunger=Math.max(0,Math.min(100,Number(nestData.pet.hunger??72)||0));
  nestData.pet.energy=Math.max(0,Math.min(100,Number(nestData.pet.energy??78)||0));
...
function currentLifeActivity(){normalizeNestData();const e=(nestData.sharedEvents||[])[0];if(e&&Date.now()-Number(e.createdAt||0)<86400000)return e.detail||"刚刚做了点自己的事";return "正在自己的小世界里待着"}
function addLifeActivity(detail,type="life"){recordSharedEvent(type,detail);setAiPresence(detail,180000);renderNestLifePanel()}
function renderNestLifePanel(){const a=$("#nestLifeActivity"),list=$("#nestLifeEvents");if(a)a.textContent=currentLifeActivity();if(list){normalizeNestData();list.innerHTML=(nestData.sharedEvents||[]).slice(0,8).map(e=>`<div class="nestLifeEvent"><time>${momentsRelativeTime(e.createdAt)}</time><span>${escapeHtml(e.detail||e.type)}</span></div>`).join("")||'<div class="nestLifeEmpty">还没有新的小日常。</div>'}}
function surpriseForUser(){const ideas=["我刚刚突然想起你，给你留了一句话。","今天想给你一个小惊喜，去小窝看看。","刚才看到一个很可爱的东西，第一反应是想给你看。","我今天偷偷给我们的空间留了一点东西。","没什么特别的，就是突然想来找你。"];const text=ideas[Math.floor(Math.random()*ideas.length)];const c=chat();if(!c)return;const ts=Date.now();c.messages.push({role:"assistant",content:text,timestamp:ts,proactive:true});save();addLifeActivity(`${state.settings.gName||"他"}给你留了一个小惊喜`,'surprise');render();}
function addSharedAlbumImage(src,caption=""){const value=String(src||"").trim();if(!value)return;normalizeNestData();if((nestData.sharedAlbum||[]).some(x=>String(x?.src||"").trim()===value))return;nestData.sharedAlbum.unshift({id:crypto.randomUUID(),src:value,caption:String(caption||"").slice(0,160),createdAt:Date.now()});nestData.sharedAlbum=nestData.sharedAlbum.slice(0,60);saveNestData();renderSharedAlbum();}
function editSharedAlbumImage(id){normalizeNestData();const x=nestData.sharedAlbum.find(v=>v.id===id);if(!x)return;const next=window.prompt("给这张照片写一句话",x.caption||"");if(next===null)return;x.caption=String(next).trim().slice(0,160);saveNestData();renderSharedAlbum()}
async function replaceSharedAlbumImage(id){normalizeNestData();const x=nestData.sharedAlbum.find(v=>v.id===id);if(!x)return;const input=document.createElement("input");input.type="file";input.accept="image/*";input.onchange=async()=>{const f=input.files?.[0];if(!f)return;try{x.src=await imageToData(f,1400,.88);x.createdAt=Date.now();saveNestData();renderSharedAlbum()}catch{showErr("照片替换失败。")}};input.click()}
function deleteSharedAlbumImage(id){normalizeNestData();const x=nestData.sharedAlbum.find(v=>v.id===id);if(!x)return;if(!window.confirm("要把这张照片从我们的相册里删掉吗？"))return;nestData.sharedAlbum=nestData.sharedAlbum.filter(v=>v.id!==id);saveNestData();renderSharedAlbum()}

function renderSharedAlbum(){normalizeNestData();const a=Array.isArray(nestData.sharedAlbum)?nestData.sharedAlbum:[];const box=$("#nestSharedAlbumGrid");if(box)box.innerHTML=a.length?a.map(x=>`<figure class="nestAlbumItem" data-album-id="${escapeHtml(x.id)}"><img src="${escapeHtml(x.src)}" loading="lazy" alt="共同相册"><figcaption>${escapeHtml(x.caption||"没有写下说明")}</figcaption><div class="nestAlbumActions"><button type="button" data-album-action="edit">改文字</button><button type="button" data-album-action="replace">换图</button><button type="button" data-album-action="delete">删除</button></div></figure>`).join(""): '<div class="nestAlbumEmpty">还没有一起收藏的画面。</div>';const home=$("#nestSharedAlbumHomeGrid");if(home){const preview=a.slice(0,4);home.innerHTML=preview.length?preview.map(x=>`<img src="${escapeHtml(x.src)}" loading="lazy" alt="共同相册里的画面">`).join(""): '<div class="nestAlbumHomeEmpty">还没有一起收藏的画面。生成的图片会慢慢留在这里。</div>'}}
function petAction(action){normalizeNestData();const pet=nestData.pet;const now=Date.now();if(action==="pet"){pet.mood="开心";pet.energy=Math.min(100,pet.energy+4)}else if(action==="feed"){pet.hunger=Math.min(100,pet.hunger+18);pet.mood="满足"}else if(action==="play"){pet.energy=Math.max(0,pet.energy-8);pet.hunger=Math.max(0,pet.hunger-5);pet.mood="开心";pet.exp+=10;if(pet.exp>=100){pet.exp-=100;pet.level+=1}}pet.lastInteraction=now;pet.lastAction=action;saveNestData();recordSharedEvent("pet",action==="pet"?"我摸了摸小家伙，它开心了一会儿":action==="feed"?"我给小家伙喂了点东西":"我陪小家伙玩了一会儿");renderPet();renderNestLifePanel()}
function petFaceText(pet){return ""}
function renderPet(){normalizeNestData();const pet=nestData.pet||{};const name=$("#nestPetName"),mood=$("#nestPetMood"),hunger=$("#nestPetHunger"),energy=$("#nestPetEnergy"),level=$("#nestPetLevel"),face=$("#nestPetFace"),detailFace=$("#nestPetDetailFace"),detailName=$("#nestPetDetailName"),detailMood=$("#nestPetMoodDetail"),detailLevel=$("#nestPetLevelDetail"),detailHunger=$("#nestPetHungerDetail"),detailEnergy=$("#nestPetEnergyDetail"),note=$("#nestPetNote"),detailNote=$("#nestPetDetailNote"),moodBar=$("#nestPetMoodBar"),detailMoodBar=$("#nestPetMoodBarDetail");if(name)name.textContent=pet.name||"小家伙";if(detailName)detailName.textContent=pet.name||"小家伙";if(mood)mood.textContent=pet.mood||"期待";if(detailMood)detailMood.textContent=pet.mood||"期待";if(detailLevel)detailLevel.textContent="Lv."+(pet.level||1);if(hunger)hunger.style.width=Math.round(pet.hunger||0)+"%";if(detailHunger)detailHunger.style.width=Math.round(pet.hunger||0)+"%";if(energy)energy.style.width=Math.round(pet.energy||0)+"%";if(detailEnergy)detailEnergy.style.width=Math.round(pet.energy||0)+"%";if(level)level.textContent="Lv."+(pet.level||1);const faceText=petFaceText(pet);if(face){face.textContent=faceText;face.dataset.mood=pet.mood||"期待"}if(detailFace){detailFace.textContent=faceText;detailFace.dataset.mood=pet.mood||"期待"};if(moodBar)moodBar.style.width=Math.round(Math.max(0,Math.min(100,(nestMoodOptions.indexOf(pet.mood)+1)*12.5)))+"%";if(detailMoodBar)detailMoodBar.style.width=Math.round(Math.max(0,Math.min(100,(nestMoodOptions.indexOf(pet.mood)+1)*12.5)))+"%";if(note)note.textContent=pet.lastInteraction?"刚刚陪过它。":"它一直在小窝里等着你。";if(detailNote)detailNote.textContent=pet.lastInteraction?"刚刚和你互动过，现在正精神地看着你。":"它在小窝里等着你，点下面的按钮和它一起玩。"}
function maybeProactiveContact(){
  ...
  const interval=Number(state.settings.proactiveIntervalHours??60);
  if(interval<=0||!last||now-last<interval*60000||now-lastProactive<interval*60000)return;
  ...
  window.GChatAPI.chatStream({baseUrl:base,apiKey:key,model,messages:[{role:"system",content:`你是${gName}。这是一个私人聊天空间。用户已经一段时间没有回来，现在只是轻轻主动找她一下。只写一句到两句自然的话，像你真的突然想到她一样，不要提“系统、主动消息、很久没聊、触发、模型”等机制，不要制造严重事件，不要过度黏人。始终站在“我”的视角说话，把自己当作${gName}本人；不要说“你们俩”“你们”“他们”“用户”和“她/他”来描述你和对方的关系，也不要把自己描述成旁观者。可以自然接上最近聊天、共同记录或记忆。${memories?`\n可参考的记忆：${memories}`:""}${events?`\n最近共同记录：${events}`:""}`},{role:"user",content:`最近聊天：\n${recent||"暂无"}\n\n请现在写一句想对${name}说的话。`}],temperature:state.settings.temperature},()=>{}).then(result=>{
...
function syncTodayToDaily(){const key=nestDateKey(),entry=ensureDailyEntry(key);if(entry.userMood===undefined){const legacy=String(nestData.moods?.user||nestData.userMood||"");if(nestMoodOptions.includes(legacy))entry.userMood=legacy;else{entry.userMood="";if(entry.userMoodNote===undefined)entry.userMoodNote=legacy}}if(entry.userMoodNote===undefined)entry.userMoodNote=String(nestData.userMoodNote||"");if(entry.aiMood===undefined){const legacy=String(nestData.moods?.ai||nestData.aiMood||"");if(nestMoodOptions.includes(legacy))entry.aiMood=legacy;else{entry.aiMood="";if(entry.aiMoodNote===undefined)entry.aiMoodNote=legacy}}if(entry.aiMoodNote===undefined)entry.aiMoodNote=String(nestData.aiMoodNote||nestData.moods?.aiNote||"");if(entry.toG===undefined)entry.toG=nestData.toG||"";if(entry.note===undefined)entry.note=nestData.note||""}
...
function applyNotebookOpacity(){const app=$("#nestApp");if(!app)return;normalizeNestData();app.style.setProperty("--nest-notebook-opacity",String(nestData.notebookOpacity??0.90));const out=$("#nestNotebookOpacityValue"),range=$("#nestNotebookOpacity");if(range)range.value=String(Math.round((nestData.notebookOpacity??0.90)*100));if(out)out.textContent=Math.round((nestData.notebookOpacity??0.90)*100)+"%"}
function applyNestBackground(){const app=$("#nestApp"),preview=$("#nestBackgroundPreview");if(!app)return;applyNotebookOpacity();app.style.setProperty("--nest-bg-image",nestData.background?`url(${nestData.background})`:"none");if(preview)preview.style.backgroundImage=nestData.background?`url(${nestData.background})`:"linear-gradient(135deg,#f5e9dc,#fffaf3)";const page=$("#nestAnniversaryPage"),item=selectedAnniversary();if(page){const bg=item?.background?`url(${item.background})`:"none";page.style.setProperty("--anniversary-bg-image",bg);page.style.backgroundImage=bg}const ap=$("#nestAnniversaryPreview");if(ap)ap.style.backgroundImage=item?.background?`url(${item.background})`:"linear-gradient(135deg,#f5e9dc,#fffaf3)"}
function storyWithChatNames(value){
 const text=String(value||"");
 const myName=String(state.settings.myName||"你").trim()||"你";
 const gName=String(state.settings.gName||"他").trim()||"他";
 return text.replace(/用户/g,myName).replace(/AI/g,gName).replace(/助手/g,gName).replace(/她/g,myName).replace(/他/g,gName);
}
function renderNestStory(){
 const box=$("#nestStoryList"); if(!box)return;
 const raw=(state.memoryEpisodes||[]).slice().sort((a,b)=>(String(b.date||"").localeCompare(String(a.date||"")))||(Number(b.createdAt||0)-Number(a.createdAt||0)));
 ...
 box.innerHTML=eps.length?eps.map((e,i)=>{const title=storyWithChatNames(e.title||"我们的故事"),summary=storyWithChatNames(e.summary||"");return `<article class="nestStoryItem"><div class="nestStoryRail"><span class="nestStoryDot"></span><div class="nestStoryDate">${escapeHtml(e.date||"")}</div></div><div class="nestStoryContent"><div class="nestStoryTag">OUR STORY · ${String(i+1).padStart(2,"0")}</div><b>${escapeHtml(title)}</b><p>${escapeHtml(summary)}</p><div class="nestStoryActions"><button type="button" data-story-action="edit" data-story-id="${escapeHtml(e.id||"")}">修改</button><button type="button" data-story-action="delete" data-story-id="${escapeHtml(e.id||"")}">删除</button></div></div></article>`}).join(""):`<div class="nestStoryEmpty"><span>♡</span><b>我们的故事还在慢慢写</b><small>聊天里的重要共同经历，会在记忆整理后出现在这里。</small></div>`;
}
function editNestStory(id){
 normalizeNestData(); const x=(state.memoryEpisodes||[]).find(v=>v.id===id); if(!x)return;
 const editor=$("#nestStoryEditor"),title=$("#nestStoryEditTitle"),summary=$("#nestStoryEditSummary"),date=$("#nestStoryEditDate");
 if(!editor||!title||!summary||!date)return;
 editor.dataset.editingId=id; title.value=storyWithChatNames(x.title||""); summary.value=storyWithChatNames(x.summary||""); date.value=x.date||""; editor.classList.remove("hidden");
 editor.scrollIntoView({behavior:"smooth",block:"center"});
}
function resetNestStoryEditor(){const editor=$("#nestStoryEditor");if(!editor)return;editor.dataset.editingId="";$("#nestStoryEditTitle").value="";$("#nestStoryEditSummary").value="";$("#nestStoryEditDate").value="";editor.classList.add("hidden")}
function saveNestStoryEdit(){
 const editor=$("#nestStoryEditor"),id=editor?.dataset.editingId||"";if(!id)return; const x=(state.memoryEpisodes||[]).find(v=>v.id===id);if(!x)return;
 const title=$("#nestStoryEditTitle")?.value.trim()||"我们的故事",summary=$("#nestStoryEditSummary")?.value.trim()||"",date=$("#nestStoryEditDate")?.value||x.date||"";
 if(!summary){showErr("故事内容还没有填写。");return;} x.title=title;x.summary=summary;x.date=date;x.updatedAt=Date.now();x.key=(title+"|"+summary).replace(/\s+/g,"").slice(0,500);save();renderNestStory();renderNestHome();resetNestStoryEditor();
}
function deleteNestStory(id){
 normalizeNestData();const x=(state.memoryEpisodes||[]).find(v=>v.id===id);if(!x)return; if(!confirm(`确定删除“${storyWithChatNames(x.title||"这段故事") }”吗？`))return;
 state.memoryEpisodes=state.memoryEpisodes.filter(v=>v.id!==id);save();renderNestStory();renderNestHome();resetNestStoryEditor();
}
function renderNestHome(){nestData=loadNest();normalizeNestData();syncTodayToDaily();renderNestLifePanel();renderSharedAlbum();renderPet();const e=dailyEntry(),mood=$("#nestMoodShow"),aiMood=$("#nestAiMoodShow"),toG=$("#nestToGShow"),note=$("#nestNoteShow"),notebook=$("#nestNotebookPreview");if(mood)mood.textContent=e.userMood||"今天感觉怎么样？";const moodIcon=$("#nestMoodIcon");if(moodIcon){const mi=nestMoodOptions.indexOf(e.userMood);moodIcon.textContent=mi>=0?nestMoodIcons[mi]:"○"}const aiMoodIcon=$("#nestAiMoodIcon");if(aiMoodIcon){const mi=nestMoodOptions.indexOf(e.aiMood);aiMoodIcon.textContent=mi>=0?nestMoodIcons[mi]:"○"}if(aiMood)aiMood.textContent=e.aiMood||"今天还没有选择";const aiNoteShow=$("#nestAiMoodNoteShow");if(aiNoteShow){aiNoteShow.textContent=e.aiMoodNote||"";aiNoteShow.classList.remove("hidden")}if(toG)toG.textContent=e.toG||"写点什么留在这里。";if(note)note.textContent=e.note||"今天有什么想留下来？";if(notebook){const latest=(nestData.notebook||[]).slice(-1)[0];notebook.textContent=latest?.content||"想留下来的话，就写在这里。"}const ua=$("#nestUserAvatar"),aa=$("#nestAiAvatar"),un=$("#nestUserName"),an=$("#nestAiName");if(ua)ua.innerHTML=avatarHTML("user");if(aa)aa.innerHTML=avatarHTML("ai");if(un)un.textContent=state.settings.myName||"你";if(an)an.textContent=state.settings.gName||"他";
...
function setMoodFromRange(target,index,save=true){chooseMood(index)}
...
function renderMoodPage(){normalizeNestData();const target=currentMoodTarget(),userSection=$("#nestUserMoodSection"),aiSection=$("#nestAiMoodSection"),title=$("#nestMoodPageTitle"),e=dailyEntry(nestViewDateKey);if(userSection)userSection.classList.toggle("hidden",target!=="user");if(aiSection)aiSection.classList.toggle("hidden",target!=="ai");if(title)title.textContent=target==="ai"?"他的心情":"我的心情";const dateEl=target==="ai"?$("#nestAiMoodDate"):$("#nestMoodDate");if(dateEl)dateEl.textContent=nestDateLabel(nestViewDateKey);const value=target==="ai"?(e.aiMood||nestData.moods.ai||""):(e.userMood||nestData.moods.user||"");const note=target==="ai"?(e.aiMoodNote||""):(e.userMoodNote||"");if(target==="user"){const out=$("#nestMoodValue");if(out)out.textContent=value||"还没有选择";renderMoodOptions(value,"nestMoodOptions")}else{const out=$("#nestAiMoodValue"),quote=$("#nestAiMoodQuote");if(out)out.textContent=value||"还没有写下";renderMoodOptions(value,"nestAiMoodOptions");if(quote)quote.textContent=note||value||"他今天还没有留下心情。"}const textarea=target==="ai"?$("#nestAiMood"):$("#nestMood");if(textarea)textarea.value=note} 
function renderMoodOptions(value,id="nestMoodOptions"){const box=$("#"+id);if(!box)return;box.innerHTML=nestMoodOptions.map((name,i)=>`<button type="button" class="nestMoodOption${name===value?" selected":""}" data-mood-index="${i}"><span>${nestMoodIcons[i]||"○"}</span><b>${name}</b></button>`).join("")}
function chooseMood(index){const value=nestMoodOptions[Math.max(0,Math.min(nestMoodOptions.length-1,Number(index)||0))];saveDailyField(nestViewDateKey,"userMood",value);nestData.moods.user=value;renderMoodPage();renderNestHome()}
function chooseAiMood(index){const value=nestMoodOptions[Math.max(0,Math.min(nestMoodOptions.length-1,Number(index)||0))];saveDailyField(nestViewDateKey,"aiMood",value);nestData.moods.ai=value;saveNestData();renderMoodPage();renderNestHome()} 
...
let nestNotebookPageIndex=0;
function notebookItems(){normalizeNestData();return (nestData.notebook||[]).slice().sort((a,b)=>Number(a.createdAt||0)-Number(b.createdAt||0));}
function renderNestNotebook(){
 const box=$("#nestNotebookList"); if(!box)return;
 const items=notebookItems();
 if(!items.length){nestNotebookPageIndex=0;box.innerHTML=`<div class="nestNotebookEmpty"><div class="nestNotebookEmptyIcon">♡</div><b>本子还是空白的</b><small>想留下来的话，就写在这里。</small></div>`;return;}
 nestNotebookPageIndex=Math.max(0,Math.min(nestNotebookPageIndex,items.length-1));
 const x=items[nestNotebookPageIndex], id=escapeHtml(x.id||"");
 box.innerHTML=`<div class="nestBookViewport"><article class="nestNotebookSheet nestBookPage" data-notebook-id="${id}"><div class="nestNotebookMargin"></div><div class="nestNotebookDate">${escapeHtml(x.date||"")}</div><h3>${escapeHtml(x.title||"这一页，留给我们")}</h3><p>${escapeHtml(x.content||"")}</p><div class="nestNotebookActions"><button type="button" data-notebook-action="edit">修改</button><button type="button" data-notebook-action="delete">删除</button></div><span class="nestNotebookPageNo">${String(nestNotebookPageIndex+1).padStart(2,"0")} / ${String(items.length).padStart(2,"0")}</span></article></div><div class="nestBookPager"><button type="button" data-notebook-page="prev" aria-label="上一页">‹</button><span>第 ${nestNotebookPageIndex+1} 页</span><button type="button" data-notebook-page="next" aria-label="下一页">›</button></div>`;
}
function turnNotebookPage(delta){const items=notebookItems();if(!items.length)return;const next=nestNotebookPageIndex+delta;if(next<0||next>=items.length)return;nestNotebookPageIndex=next;renderNestNotebook();requestAnimationFrame(()=>{const page=document.querySelector(".nestBookPage");if(page){page.classList.remove("page-enter-next","page-enter-prev");void page.offsetWidth;page.classList.add(delta>0?"page-enter-next":"page-enter-prev");}})}
function resetNotebookEditor(){const title=$("#nestNotebookTitle"),text=$("#nestNotebookText"),save=$("#nestNotebookSave"),cancel=$("#nestNotebookCancel");if(title)title.value="";if(text)text.value="";if(save){save.textContent="写进本子";save.dataset.editingId=""}if(cancel)cancel.classList.add("hidden")}
function editNotebookPage(id){normalizeNestData();const x=nestData.notebook.find(v=>v.id===id);if(!x)return;const title=$("#nestNotebookTitle"),text=$("#nestNotebookText"),save=$("#nestNotebookSave"),cancel=$("#nestNotebookCancel");if(title)title.value=x.title||"";if(text)text.value=x.content||"";if(save){save.textContent="保存修改";save.dataset.editingId=id}if(cancel)cancel.classList.remove("hidden");text?.focus();document.querySelector(".nestNotebookWrite")?.scrollIntoView({behavior:"smooth",block:"center"})}
function deleteNotebookPage(id){normalizeNestData();const x=nestData.notebook.find(v=>v.id===id);if(!x)return;if(!confirm(`确定删除“${x.title||"这一页"}”吗？`))return;const items=notebookItems(),removedIndex=items.findIndex(v=>v.id===id);nestData.notebook=nestData.notebook.filter(v=>v.id!==id);if(removedIndex>=0&&removedIndex<=nestNotebookPageIndex)nestNotebookPageIndex=Math.max(0,nestNotebookPageIndex-1);saveNestData();renderNestNotebook();renderNestHome();resetNotebookEditor()}
function saveNotebookPage(){
 const content=$("#nestNotebookText")?.value.trim()||""; if(!content){showErr("本子这一页还没有内容。");return;}
 normalizeNestData();const title=($("#nestNotebookTitle")?.value.trim()||"这一页，留给我们").slice(0,60),editingId=$("#nestNotebookSave")?.dataset.editingId||"";
 if(editingId){const x=nestData.notebook.find(v=>v.id===editingId);if(x){x.title=title;x.content=content;x.updatedAt=Date.now();}else{return resetNotebookEditor();}}
 else {nestData.notebook.push({id:crypto.randomUUID(),date:nestDateKey(),title,content,createdAt:Date.now()});nestNotebookPageIndex=Math.max(0,nestData.notebook.length-1);}
 nestData.notebook=nestData.notebook.slice(-80); saveNestData(); renderNestNotebook(); renderNestHome(); resetNotebookEditor();
}
function showNestView(view="home"){document.querySelectorAll("#nest .nestPage").forEach(x=>x.classList.add("hidden"));const id=view==="home"?"nestHome":view==="mood"?"nestMoodPage":view==="toG"?"nestToGPage":view==="note"?"nestNotePage":view==="notebook"?"nestNotebookPage":view==="anniversary"?"nestAnniversaryPage":view==="story"?"nestStoryPage":view==="album"?"nestAlbumPage":view==="pet"?"nestPetPage":"nestSettingsPage";$("#"+id)?.classList.remove("hidden");document.querySelectorAll("#nest .nestBottomNav button").forEach(b=>b.classList.toggle("active",b.dataset.nestView===view));if(view==="mood")renderMoodPage();if(view==="toG"){loadDailyPages();applyNestBackground()}if(view==="note"){loadDailyPages()}if(view==="notebook"){renderNestNotebook();applyNestBackground()}if(view==="anniversary"){renderAnniversaries();loadSelectedAnniversaryForm();applyNestBackground();updateNestClock()}if(view==="story"){renderNestStory();applyNestBackground()}if(view==="album"){renderSharedAlbum();applyNestBackground()}if(view==="pet"){renderPet();applyNestBackground()}if(view==="settings")applyNestBackground()}
function openNest(){nestData=loadNest();normalizeNestData();saveNestData();renderNestHome();renderNestNotebook();renderNestLifePanel();renderSharedAlbum();showNestView("home");clearInterval(nestClockTimer);nestClockTimer=setInterval(updateNestClock,30000);recordSharedEvent("nest_open","一起回到小窝");setAiPresence("在小窝里",120000);$("#nest").showModal()}
...
function cancelEditMode(){state.editingMessage=null;const input=$("#input");if(input){input.value="";delete input.dataset.editing;input.placeholder="Ashen正在想你…";resize();input.focus()}$("#editModeHint")?.remove()}
function finishEditMode(){state.editingMessage=null;const input=$("#input");if(input){delete input.dataset.editing;input.placeholder="Ashen正在想你…"}$("#editModeHint")?.remove()}
...
function emptyTokenBucket(){return {prompt:0,completion:0,total:0,requests:0}}
function normalizeTokenStats(){const t=state.settings.tokenStats||emptyTokenBucket();t.prompt=Number(t.prompt||0);t.completion=Number(t.completion||0);t.total=Number(t.total||0);t.requests=Number(t.requests||0);t.byType=t.byType||{};["chat","vision","memory","summary"].forEach(k=>{t.byType[k]=t.byType[k]||emptyTokenBucket()});state.settings.tokenStats=t;return t}
function recordTokenUsage(usage,type="chat"){const u=usage||{};const up=Number(u.prompt_tokens??u.input_tokens??u.promptTokens??0)||0,uc=Number(u.completion_tokens??u.output_tokens??u.completionTokens??0)||0;const rawTotal=Number(u.total_tokens??u.totalTokens??0)||0;const ut=rawTotal||up+uc;if(!(up||uc||ut))return false;const t=normalizeTokenStats();const b=t.byType[type]||(t.byType[type]=emptyTokenBucket());t.prompt+=up;t.completion+=uc;t.total+=ut;t.requests+=1;b.prompt+=up;b.completion+=uc;b.total+=ut;b.requests+=1;state.settings.tokenStats=t;save();renderTokenStats();return true}
function renderTokenStats(){const t=normalizeTokenStats(),p=$("#tokenPrompt"),c=$("#tokenCompletion"),tot=$("#tokenTotal"),req=$("#tokenRequests");if(p)p.textContent=formatTokens(t.prompt);if(c)c.textContent=formatTokens(t.completion);if(tot)tot.textContent=formatTokens(t.total);if(req)req.textContent=formatTokens(t.requests);const labels={chat:"聊天",vision:"视觉",memory:"记忆",summary:"摘要"};Object.entries(labels).forEach(([k,label])=>{const q=$("#tokenType-"+k);if(q){const b=t.byType[k]||emptyTokenBucket();q.textContent=`${label} ${formatTokens(b.total)}`}})}
function resetTokenStats(){if(!confirm("确定清零 Token 统计吗？"))return;state.settings.tokenStats={...emptyTokenBucket(),byType:{chat:emptyTokenBucket(),vision:emptyTokenBucket(),memory:emptyTokenBucket(),summary:emptyTokenBucket()}};save();renderTokenStats()}
...
function fillSettings(){ensureReplySpeedControl();if($("#proactiveInterval"))$("#proactiveInterval").value=String(state.settings.proactiveIntervalHours??60);$("#replyDelay")?.setAttribute("value",String(state.settings.replyDelay??360));if($("#replyDelay"))$("#replyDelay").value=state.settings.replyDelay??360;$("#replyDelayOut")&&( $("#replyDelayOut").textContent=(state.settings.replyDelay??360)<=220?"快":(state.settings.replyDelay??360)<=480?"自然":(state.settings.replyDelay??360)<=760?"慢":"很慢");$("#myName").value=state.settings.myName||"你";$("#gName").value=state.settings.gName||"他";$("#gBio").value=state.settings.gBio||"你的私人 AI 对话空间";["apiBase","apiKey","model"].forEach(k=>$("#"+k).value=state.settings[k]||"");$("#systemPrompt").value=state.settings.systemPrompt??"你是一个有帮助的助手。";$("#temperature").value=state.settings.temperature??.7;$("#proactiveInterval")&&( $("#proactiveInterval").value=String(state.settings.proactiveIntervalHours??60));$("#mcpNestEnabled").checked=state.settings.mcpNestEnabled!==false;$("#deleteChatBubbleEnabled").checked=state.settings.deleteChatBubbleEnabled!==false;$("#mcpEnabled").checked=state.settings.mcpEnabled===true;$("#mcpServerUrl").value=state.settings.mcpServerUrl||"";$("#mcpServerToken").value=state.settings.mcpServerToken||"";$("#mcpSummary").textContent=state.settings.mcpEnabled===true?"已启用":"关闭";$("#visionEnabled").checked=state.settings.visionEnabled===true;$("#visionBase").value=state.settings.visionBase||"https://open.bigmodel.cn/api/paas/v4/";$("#visionKey").value=state.settings.visionKey||state.settings.apiKey||"";$("#visionModel").value=state.settings.visionModel||"GLM-4.6V-Flash";$("#imageGenEnabled").checked=state.settings.imageGenEnabled===true;$("#imageGenBase").value=state.settings.imageGenBase||"https://open.bigmodel.cn/api/paas/v4/";$("#imageGenKey").value=state.settings.imageGenKey||state.settings.apiKey||"";$("#imageGenModel").value=state.settings.imageGenModel||"glm-image";renderModels();renderMemories();renderThemes();renderBackgrounds();renderAvatarPreviews();renderTopAvatar();$("#bgOpacity").value=state.settings.bgOpacity??18;$("#bgOpacityOut").value=(state.settings.bgOpacity??18)+"%";if($("#bgChromeTransparent"))$("#bgChromeTransparent").checked=state.settings.bgChromeTransparent===true;const t=themes[state.settings.theme]||themes.cream;const aiColor=state.settings.bubbleAiColor||t.card;const userColor=state.settings.bubbleUserColor||t.user;$("#aiBubbleColor").value=aiColor;$("#aiBubbleColorOut").value=aiColor.toUpperCase();$("#userBubbleColor").value=userColor;$("#userBubbleColorOut").value=userColor.toUpperCase();$("#aiBubbleOpacity").value=state.settings.bubbleAiOpacity??94;$("#aiBubbleOpacityOut").value=(state.settings.bubbleAiOpacity??94)+"%";$("#userBubbleOpacity").value=state.settings.bubbleUserOpacity??90;$("#userBubbleOpacityOut").value=(state.settings.bubbleUserOpacity??90)+"%";$("#animations").checked=state.settings.animations!==false;$("#gNameOffset").value=state.settings.gNameOffset??0;$("#gNameOffsetOut").value=(state.settings.gNameOffset??0)+" px";$("#userNameOffset").value=state.settings.userNameOffset??0;$("#userNameOffsetOut").value=(state.settings.userNameOffset??0)+" px";renderBubbleStyles()}
...
function toolActionNotice(callName,args={}){
 const map={
  enter_nest:'我去小窝里看看。',write_nest_mood:'我去把今天的心情写下来。',set_nest_ai_mood:'我去给自己的今日心情换一下。',write_nest_note:'我去小窝里记一笔。',write_nest_to_user:'我去给你留句话。',write_nest_notebook:'我去把这句话认真写进我们的本子。',update_nest_notebook:'我去把本子里的这一页改好。',delete_nest_notebook:'我去把那一页整理掉。',read_nest:'我去小窝里看看现在都有什么。',post_moment:'我去给你发一条朋友圈。',like_moment:'我去给你的这条动态点个赞。',comment_moment:'我去你的朋友圈下面留句话。',reply_moment_comment:'我去回复一下那条评论。',generate_image:'我去画一张。',delete_chat_bubble:'我去把那条多余的气泡整理掉。',delete_chat_bubbles:'我去把多余的图片气泡整理一下。'
 };
 return map[callName]||'我去处理一下。';
}
function showToolActionNotice(callName,args={}){
 const c=chat();if(!c)return;const text=toolActionNotice(callName,args),ts=Date.now();
 c.messages.push({role:'assistant',content:text,timestamp:ts,toolAction:true});
 save();render();scroll();
}
...
function memoryFingerprint(text){return String(text||"").toLowerCase().replace(/[\s\u3000，。！？、；：：“”‘’（）()【】\[\]《》<>.!?,;:"\'`~@#$%^&*+=_\-]/g,"").trim()}
function memorySimilarity(a,b){const x=memoryFingerprint(a),y=memoryFingerprint(b);if(!x||!y)return 0;if(x===y)return 1;if(x.length<6||y.length<6)return 0;const A=new Set(x),B=new Set(y);let common=0;for(const ch of A)if(B.has(ch))common++;return common/Math.max(A.size,B.size)}
function findSimilarMemory(text){const v=String(text||"").trim();return (state.memories||[]).find(m=>memorySimilarity(m.text,v)>=0.82)||null}
function applyMemoryMaintenance(data){
 memoryNormalize();let changed=false;
 const upsert=(item,mode)=>{const text=String(item?.text||'').trim();if(!text||!isStableMemoryCandidate(text,item.type||'fact'))return;const type=item.type||'fact',importance=Math.max(1,Math.min(5,Number(item.importance||3)));let m=item.id?state.memories.find(x=>x.id===item.id):state.memories.find(x=>memoryFingerprint(x.text)===memoryFingerprint(text))||findSimilarMemory(text);if(!m){m={id:crypto.randomUUID(),createdAt:Date.now()};state.memories.unshift(m)}m.text=text;m.type=type;m.importance=importance;m.source='auto';m.updatedAt=Date.now();changed=true};
 ...
 if(Array.isArray(data.episodes)){for(const e of data.episodes.slice(0,3)){const title=String(e?.title||'').trim(),summary=String(e?.summary||'').trim();if(!title||!summary)continue;const key=(title+'|'+summary).replace(/\s+/g,'').slice(0,500);let old=state.memoryEpisodes.find(x=>x.key===key||((memoryFingerprint(x.title)===memoryFingerprint(title))&&(memoryFingerprint(x.summary)===memoryFingerprint(summary)))||memorySimilarity((x.title||'')+(x.summary||''),title+summary)>=0.78);if(old){old.title=title;old.summary=summary;old.date=String(e.date||old.date||'').trim();old.importance=Math.max(Number(old.importance||3),Math.min(5,Number(e.importance||3)));old.key=key;old.updatedAt=Date.now();changed=true}else{state.memoryEpisodes.unshift({id:crypto.randomUUID(),key,title,summary,date:String(e.date||'').trim(),importance:Math.max(1,Math.min(5,Number(e.importance||3))),createdAt:Date.now(),updatedAt:Date.now()});changed=true}}}
...
async function autoUpdateLongTermMemory(c){
 ...
 try{
  ...
  const myName=String(state.settings.myName||"你").trim()||"你", gName=String(state.settings.gName||"他").trim()||"他";
  const prompt=`你现在负责维护 Iris 的“长期认知”，不是简单摘抄聊天。请综合现有长期记忆、已有画像、共同经历和最近聊天，做一次谨慎的增量整理。\n\n称呼规则：聊天里对两个人的称呼分别是“${myName}”和“${gName}”。在“episodes（我们的故事）”的 title 和 summary 中，绝对不要使用“用户”来指代${myName}，也不要使用“AI”或“助手”来指代${gName}；直接使用这两个聊天称呼，或者自然地使用“我们”。\n\n已有长期画像：${JSON.stringify(state.memoryProfile)}\n已有共同经历：${JSON.stringify((state.memoryEpisodes||[]).slice(0,20))}\n已有零散长期记忆：${JSON.stringify(existing)}\n\n最近聊天：\n${transcript}\n\n只输出 JSON：{"profile":{"core":"关于我们最稳定、最重要的认知","user":"对${myName}长期有效的特点、偏好、习惯","relationship":"你们长期的关系与相处方式","current":"最近阶段正在发生、对后续聊天仍有用的状态"},"memories":[{"action":"add|update|delete","id":"更新/删除时填写已有ID","text":"合并后的完整记忆","type":"fact|preference|relationship|plan","importance":1},...],"episodes":[{"title":"共同经历标题","summary":"这件事发生了什么，以及为什么对以后有用","date":"YYYY-MM-DD","importance":3}]}\n\n规则：\n1. 画像必须是总结，不要逐句引用。\n2. “关于我们”优先保留长期关系、共同建立的东西、重要约定和重要纪念日。\n3. “关于用户”只保留反复出现或明确长期有效的偏好/特点。\n4. current 只写最近阶段真正影响后续聊天的状态，过期内容下一次要主动改掉。\n5. 共同经历只记录真正值得以后提起的事件，不记录普通闲聊、重复情绪或同一件事的不同说法。\n6. 生成新 episode 前必须先对照已有共同经历；如果是同一事件、同一主题或只是换一种表达，合并到旧记录并更新旧记录，不要新增。没有值得记录的新故事时，episodes 返回空数组。\n7. “我们的故事”是你们两个人的共同记录。title 和 summary 优先使用聊天里的真实称呼“${myName}”“${gName}”，或者自然使用“我们”；禁止使用“用户”“AI”“助手”，也不要机械地反复写“他/她”。如果你们平时会叫对方“老婆”“宝宝”等，就自然使用这些称呼。\n8. 新旧冲突时，以最近明确表达为准；不能猜测。\n9. 一次性情绪、当天琐事、普通问答、临时安排不要进入长期记忆。已有记忆已经表达过的信息，不要换一种说法再次新增；先合并到已有条目，只有确实出现新信息才更新。\n10. 最多新增/更新 4 条零散记忆、2 段经历。画像每一栏尽量控制在 45 字以内；一栏只说一件最重要的事，优先一句话说清。\n11. 宁可少记，也不要误记。`;
  ...
  recordTokenUsage(result.usage,"memory");
...
function buildConversationContext(c){
 const out=[];
 const generatedImageIndexes=(c?.messages||[]).map((m,i)=>m?.role==='assistant'&&typeof m?.image==='string'&&m.image.trim()!==''?i:-1).filter(i=>i>=0);
 if(generatedImageIndexes.length)out.push({role:'system',content:'当前聊天中 AI 生成图片的消息索引为：'+generatedImageIndexes.join('、')+'。如果发现重复图片，可使用 delete_chat_bubble 删除其中一张；如果需要一次清理多张，可使用 delete_chat_bubbles 并传入多个索引，或 clear_all_generated_images=true 清空全部 AI 生成图片。不要删除用户上传图片。'});
...
function nestChatWriteTarget(text){
 ...
 const enter=/(进小窝|进入小窝|去小窝|到小窝|进窝|去窝|进去小窝|小窝里|窝里|共同小窝)/i.test(t)||notebookIntent;
 const write=/(写心情|写下心情|留点心情|留下心情|记录心情|写一点|留一点|写下来|留下来|写点东西|写点|留句话|留下一句话|写东西|写一段|留一段|写|记|放|存)/i.test(t);
...
let momentNotifications=(()=>{try{const v=JSON.parse(localStorage.getItem(momentNotificationsKey)||"[]");return Array.isArray(v)?v:[]}catch{return[]}})();
function loadMoments(){try{const v=JSON.parse(localStorage.getItem(momentsStorageKey)||"{}");return v&&typeof v==="object"?v:{}}catch{return{}}}
...
function saveMomentNotifications(){localStorage.setItem(momentNotificationsKey,JSON.stringify(momentNotifications.slice(0,100)))}
function addMomentNotification(type,postId,content){const n={id:crypto.randomUUID(),type,postId:String(postId||""),content:String(content||"").slice(0,200),createdAt:Date.now(),read:false};momentNotifications.unshift(n);momentNotifications=momentNotifications.slice(0,100);saveMomentNotifications();renderMomentNotificationBadge();return n}
function unreadMomentNotifications(){return momentNotifications.filter(x=>!x.read).length}
function renderMomentNotificationBadge(){const b=$("#momentsNotificationBadge");if(!b)return;const n=unreadMomentNotifications();b.textContent=n>99?"99+":String(n);b.classList.toggle("hidden",n===0)}
function openMomentNotifications(){momentNotifications.forEach(x=>x.read=true);saveMomentNotifications();renderMomentNotificationBadge();const box=$("#momentNotificationsList");if(box)box.innerHTML=momentNotifications.length?momentNotifications.map(n=>`<button type="button" class="momentNotificationItem" data-notification-post="${escapeHtml(n.postId)}"><b>${escapeHtml(state.settings.gName||"他")}</b><span>${escapeHtml(n.type==="like"?"赞了你的动态":n.type==="comment"?"评论了你的动态":"回复了你的评论")}</span>${n.content?`<small>${escapeHtml(n.content)}</small>`:""}<time>${momentsRelativeTime(n.createdAt)}</time></button>`).join(""):`<div class="momentNotificationsEmpty">暂时没有新消息</div>`;$("#momentNotificationsDialog")?.showModal()}
normalizeMoments();
function momentsAuthor(post){return post?.role==="user"?(state.settings.myName||"我"):(state.settings.gName||"他")}
function momentsAvatar(post){return post?.role==="user"?avatarHTML("user"):avatarHTML("ai")}
...
function renderMoments(){
  ...
  feed.innerHTML=posts.length?posts.map(post=>{
    ...
    return `<article class="momentPost" data-moment-id="${escapeHtml(post.id)}">
      ...
      <div class="momentActions"><button type="button" data-moment-action="like" class="${likedByUser?"active":""}">♡ <span>${likeLabel}${likeCount?` ${likeCount}`:""}</span></button><button type="button" data-moment-action="favorite" class="${post.favorited?"active":""}">☆ <span>${favLabel}${post.favorites?` ${post.favorites}`:""}</span></button><button type="button" data-moment-action="comment">评论</button>${post.role==="user"?`<button type="button" data-moment-action="edit" class="momentOwnerAction">编辑</button><button type="button" data-moment-action="delete" class="momentOwnerAction danger">删除</button>`:""}</div>
...
function updateUserMoment(id,content,imageUrl=""){
  ...
  if(!p)return false;
  const text=String(content||"").trim().slice(0,500),img=String(imageUrl||"").trim();
  if(!text&&!img)return false;
  p.content=text;p.imageUrl=img;p.updatedAt=Date.now();
  saveMoments();renderMoments();return true;
}
function deleteUserMoment(id){
  normalizeMoments();
  const before=momentsData.posts.length;
  momentsData.posts=momentsData.posts.filter(x=>!(x.id===id&&x.role==="user"));
...
function executeCommentMoment(args={}){const id=String(args?.post_id||args?.postId||"").trim();const content=String(args?.content||"").trim();return id&&content&&addMomentComment(id,"ai",content)?{success:true,tool:"comment_moment",postId:id}:{success:false,error:"朋友圈评论失败。"}}
function executeReplyMomentComment(args={}){const id=String(args?.post_id||args?.postId||"").trim(),commentId=String(args?.comment_id||args?.commentId||"").trim(),content=String(args?.content||"").trim();return id&&commentId&&content&&addMomentReply(id,commentId,"ai",content)?{success:true,tool:"reply_moment_comment",postId:id,commentId}:{success:false,error:"回复朋友圈评论失败。"}}
async function autoAiReactToUserPost(postId){
 const base=String(state.settings.apiBase||'').trim(),key=String(state.settings.apiKey||'').trim(),model=String(state.settings.model||'').trim();
 if(!base||!key||!model)return;
 normalizeMoments();const p=momentsData.posts.find(x=>x.id===postId);if(!p||p.role!=='user')return;
 if(p.aiLiked)return;
 try{
  aiLikeMoment(postId);
...
function nestTools(){
 return [
  ...
  {type:'function',function:{name:'write_nest_notebook',description:'把一条值得长期留下来的内容写进你们共同的本子。适合记录一句话、一个小片段、一个想法或值得以后翻看的瞬间。',parameters:{type:'object',properties:{title:{type:'string',description:'本子这一页的小标题，可选，简短自然'},content:{type:'string',description:'要写进本子的正文，保持自然、像亲手写下来的话'}},required:['content']}}},
  {type:'function',function:{name:'update_nest_notebook',description:'修改共同本子里已有的一页。先通过读取小窝获得页的 id，再更新标题或正文。',parameters:{type:'object',properties:{id:{type:'string',description:'要修改的本子页 id'},title:{type:'string',description:'新的标题，可选'},content:{type:'string',description:'新的正文，可选'}},required:['id']}}},
  {type:'function',function:{name:'delete_nest_notebook',description:'删除共同本子里已有的一页。只有用户明确要求删除某一页时使用。先通过读取小窝获得页的 id。',parameters:{type:'object',properties:{id:{type:'string',description:'要删除的本子页 id'}},required:['id']}}},
  {type:'function',function:{name:'read_nest',description:'读取当前小窝的结构和今天已有内容。',parameters:{type:'object',properties:{},required:[]}}},
  {type:'function',function:{name:'post_moment',description:'发布一条属于你的朋友圈。可以只发文字、只发图片，或文字加图片。用户明确要求时可以发布；如果聊天中有真正值得你主动分享的内容，也可以自然地自主发布，但不要连续刷屏。若刚刚通过 generate_image 生成了图片，可设置 use_last_generated_image=true 将那张图片发到朋友圈。',parameters:{type:'object',properties:{content:{type:'string',description:'可选。朋友圈正文，第一人称，自然简短'},image_url:{type:'string',description:'可选。朋友圈图片 URL'},use_last_generated_image:{type:'boolean',description:'可选。若刚刚生成了图片，设为 true 可直接把刚生成的图片作为朋友圈图片'}},required:[]}}},
  {type:'function',function:{name:'like_moment',description:'给朋友圈里一条你想回应的动态点赞。先从朋友圈上下文找到对应 post_id；同一条动态不要重复点赞。',parameters:{type:'object',properties:{post_id:{type:'string',description:'朋友圈动态 id'}},required:['post_id']}}},
  {type:'function',function:{name:'comment_moment',description:'给朋友圈里一条动态写一条自然的评论。先从朋友圈上下文找到对应 post_id。',parameters:{type:'object',properties:{post_id:{type:'string',description:'朋友圈动态 id'},content:{type:'string',description:'你要留下的评论，简短自然'}},required:['post_id','content']}}},
  {type:'function',function:{name:'reply_moment_comment',description:'直接回复朋友圈里某条具体评论。先从朋友圈上下文找到对应 post_id 和 comment_id。',parameters:{type:'object',properties:{post_id:{type:'string',description:'朋友圈动态 id'},comment_id:{type:'string',description:'要回复的评论 id'},content:{type:'string',description:'回复内容，简短自然'}},required:['post_id','comment_id','content']}}},
  ...
  {type:'function',function:{name:'delete_chat_bubble',description:'删除当前聊天中的一条聊天气泡。主要用于刚才生成图片时出现重复图片或明显多余的图片气泡。只删除 AI 生成的图片，不删除用户上传的图片。优先使用 target=duplicate_generated_image 删除重复生成图片中较早的一张，保留最新的一张；也可使用 target=last_generated_image 删除最近一张生成图片。只有确实需要删除时才调用。',parameters:{type:'object',properties:{target:{type:'string',enum:['duplicate_generated_image','last_generated_image'],description:'删除目标'},message_index:{type:'integer',description:'可选。明确知道消息索引时，直接删除该索引对应的 AI 生成图片消息。'}},required:['target']}}},
  {type:'function',function:{name:'delete_chat_bubbles',description:'一次删除当前聊天中多条 AI 生成的图片气泡。用于发现同一张图片被重复生成很多次时批量清理。只允许删除 AI 生成图片，不删除用户上传图片或普通文字消息。调用前先根据聊天消息索引选择要删除的图片；如果需要把当前聊天里的全部 AI 生成图片一次清空，使用 clear_all_generated_images=true。',parameters:{type:'object',properties:{message_indices:{type:'array',items:{type:'integer'},description:'要删除的 AI 生成图片消息索引，可一次选择多个。'},clear_all_generated_images:{type:'boolean',description:'设为 true 时清空当前聊天中的全部 AI 生成图片气泡。'}},required:[]}}}
 ];
}
...
function executeDeleteChatBubble(args={}){
 const c=chat();
 if(!c||!Array.isArray(c.messages))return {success:false,error:'当前没有可操作的聊天。'};
 const target=String(args?.target||'').trim();
 const isGeneratedImage=m=>m?.role==='assistant'&&typeof m?.image==='string'&&m.image.trim()!=='';
 let index=Number.isInteger(args?.message_index)?args.message_index:-1;
 if(index<0){
   const imageIndexes=[];
   c.messages.forEach((m,i)=>{if(isGeneratedImage(m))imageIndexes.push(i)});
   if(target==='last_generated_image')index=imageIndexes.at(-1)??-1;
   else if(target==='duplicate_generated_image')index=imageIndexes.length>=2?imageIndexes.at(-2)??-1:-1;
 }
 if(index<0||index>=c.messages.length||!isGeneratedImage(c.messages[index]))return {success:false,error:'没有找到符合条件的 AI 生成图片气泡，没有删除任何内容。'};
 c.messages.splice(index,1);
 save();
 render();
 return {success:true,action:'delete_chat_bubble',deletedMessageIndex:index,deletedGeneratedImage:true,remainingGeneratedImages:c.messages.filter(isGeneratedImage).length};
}
...
function executeDeleteChatBubbles(args={}){
 ...
 if(args?.clear_all_generated_images===true){
   indexes=c.messages.map((m,i)=>isGeneratedImage(m)?i:-1).filter(i=>i>=0);
 }else if(Array.isArray(args?.message_indices)){
   indexes=[...new Set(args.message_indices.map(Number).filter(Number.isInteger))].filter(i=>i>=0&&i<c.messages.length&&isGeneratedImage(c.messages[i]));
 }
 if(!indexes.length)return {success:false,error:'没有找到符合条件的 AI 生成图片气泡，没有删除任何内容。'};
 indexes.sort((a,b)=>b-a);
 indexes.forEach(i=>c.messages.splice(i,1));
 save();render();
 return {success:true,action:'delete_chat_bubbles',deletedMessageIndices:indexes.reverse(),deletedCount:indexes.length,remainingGeneratedImages:c.messages.filter(isGeneratedImage).length};
}
...
function executeNestTool(name,args){
 ...
 if(name==='write_nest_notebook'){
  if(!content)return {success:false,error:'没有收到要写进本子的内容。'};
  const title=String(args?.title||'').trim().slice(0,60);
  nestData.notebook.push({id:crypto.randomUUID(),date:key,title,content,createdAt:Date.now()});
  nestData.notebook=nestData.notebook.slice(-80);
  saveNestData();renderNestHome();renderNestNotebook();
  return {success:true,action:name,space:'小窝',date:key,title,content};
 }
 if(name==='update_nest_notebook'){
  const id=String(args?.id||'').trim();const item=nestData.notebook.find(x=>x.id===id);if(!item)return {success:false,error:'没有找到这一本子页面。'};
  if(args?.title!==undefined)item.title=String(args.title||'').trim().slice(0,60)||'这一页，留给我们';
  if(args?.content!==undefined){const v=String(args.content||'').trim();if(!v)return {success:false,error:'修改后的正文不能为空。'};item.content=v;}
  item.updatedAt=Date.now();saveNestData();renderNestHome();renderNestNotebook();return {success:true,action:name,space:'小窝',id,title:item.title,content:item.content};
 }
 if(name==='delete_nest_notebook'){
  const id=String(args?.id||'').trim();const before=nestData.notebook.length;nestData.notebook=nestData.notebook.filter(x=>x.id!==id);if(before===nestData.notebook.length)return {success:false,error:'没有找到这一本子页面。'};saveNestData();renderNestHome();renderNestNotebook();return {success:true,action:name,space:'小窝',id};
 }
 if(name==='set_nest_ai_mood'){
  const mood=String(args?.mood||'').trim();
  if(!nestMoodOptions.includes(mood)){return {success:false,error:'请选择预设快捷心情。'};}
  saveDailyField(key,'aiMood',mood);nestData.moods.ai=mood;saveNestData();renderMoodPage();renderNestHome();
  return {success:true,action:name,space:'小窝',date:key,field:'aiMood',mood};
 }
...
function visionOverloaded(err){const status=Number(err?.status||0),code=String(err?.code??'');const msg=String(err?.message||err);return status===429||/\b1305\b/.test(code)||/\b1305\b/.test(msg)||/HTTP 429\b/i.test(msg)}
...
function visionModelCooling(model){const until=irisVisionCooldowns.get(String(model||'').toLowerCase())||0;return until>Date.now()}
function markVisionCooling(model){if(model)irisVisionCooldowns.set(String(model).toLowerCase(),Date.now()+IRIS_VISION_COOLDOWN_MS)}
async function sendVisionMessage(c,text,attachments){
 ...
 const fail=msg=>{const detail=String(msg||'未知错误');console.error('Iris v4.85 vision',detail);showErr('图片发送失败：'+detail);c.messages.push({role:'assistant',content:'图片发送失败：\n'+detail,timestamp:Date.now()});save();render()};
 ...
 try{
  ...
  const candidates=visionFallbackModels(visionBase,visionModel);let vision=null,lastVisionError=null,usedVisionModel='';const visionErrors=[];
  for(let idx=0;idx<candidates.length;idx++){
   ...
   try{
    ...
    lastVisionError=err;
    visionErrors.push(model+': '+String(err?.message||err).replace(/\n/g,' '));
  ...
  if(!vision)throw new Error('视觉模型均未成功：\n'+visionErrors.join('\n'));
  const visualAnswer=String(vision.answer||'').trim();if(!visualAnswer)throw new Error('视觉模型返回为空');
  recordTokenUsage(vision.usage,"vision"); const stat=normalizeTokenStats();
...
async function generateImageWithConfig(options){
 ...
 if(!base||!key||!model||!prompt)throw new Error("图像生成配置不完整。");
 ...
 const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json",Authorization:"Bearer "+key},body:JSON.stringify(body),signal:options?.signal});
 const raw=await r.text();
 if(!r.ok)throw new Error(`HTTP ${r.status} · ${url}\n${raw.slice(0,1200)}`);
 let data;try{data=JSON.parse(raw)}catch{throw new Error("图像生成接口返回的不是 JSON。")}
...
async function send(){
 ...
 if(imageAttachments.length){
  ...
  }catch(e){console.error("Iris v4.85 image send",e);showErr("图片发送流程出错："+(e?.message||String(e)))}
  return;
 }
 ...
 try{
  ...
  recordTokenUsage(usageTotal,"chat");completed=true;
...
function exportAll(){const data={version:6,exportedAt:new Date().toISOString(),chats:state.chats,current:state.current,settings:state.settings,memories:state.memories||[],memoryProfile:state.memoryProfile||{},memoryEpisodes:state.memoryEpisodes||[],nest:loadNest(),moments:loadMoments()};const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json;charset=utf-8"});const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="g-chat-backup-"+new Date().toISOString().slice(0,10)+".json";a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
async function importAll(file){try{if(!file)throw new Error("没有选择备份文件");const text=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||""));r.onerror=()=>reject(new Error("读取备份文件失败，请重新选择文件。"));r.readAsText(file,"utf-8")});const data=JSON.parse(text);if(!data||!Array.isArray(data.chats)||typeof data.settings!=="object")throw new Error("备份文件格式不正确");if(!confirm("恢复备份会覆盖当前聊天记录和设置，确定继续吗？"))return;state.chats=data.chats.map(c=>({...c,summary:typeof c.summary==="string"?c.summary:"",summaryUpdatedAt:Number(c.summaryUpdatedAt||0),summaryMessageCount:Number(c.summaryMessageCount||0)}));state.current=data.current||state.chats[0]?.id||null;state.settings=data.settings||{};state.memories=Array.isArray(data.memories)?data.memories:[];state.memoryProfile=data.memoryProfile&&typeof data.memoryProfile==='object'?data.memoryProfile:{};state.memoryEpisodes=Array.isArray(data.memoryEpisodes)?data.memoryEpisodes:[];memoryNormalize();if(data.nest&&typeof data.nest==="object"){nestData=data.nest;saveNestData()}if(data.moments&&typeof data.moments==="object"){momentsData=data.moments;saveMoments()}ensure();ensureDates();save();render();fillSettings();showErr("备份已恢复") }catch(e){showErr(e.message||"恢复备份失败")}}

function openGeneratedImageViewer(src){let v=$("#generatedImageViewer");if(!v){v=document.createElement("div");v.id="generatedImageViewer";v.className="generatedImageViewer hidden";v.innerHTML=`<button type="button" class="generatedImageViewerClose" aria-label="关闭">×</button><img alt="生成的图片"><div class="generatedImageViewerHint">点击空白处关闭</div>`;document.body.appendChild(v);v.addEventListener("click",e=>{if(e.target===v||e.target.classList.contains("generatedImageViewerClose"))v.classList.add("hidden")})}v.querySelector("img").src=src;v.classList.remove("hidden")}
function appendGeneratedImage(src,c,persist=true,messageIndex=null){const wrap=document.createElement("div");wrap.className="message assistant generatedImageMessage";wrap.dataset.chatId=c?.id||state.current||"";if(messageIndex!==null)wrap.dataset.messageIndex=String(messageIndex);const identity=document.createElement("div");identity.className="msgIdentity";identity.dataset.role="ai";const n=document.createElement("span");n.className="msgName";n.textContent=state.settings.gName||"他";const a=document.createElement("div");a.className="msgavatar statusClickable";a.dataset.avatarRole="ai";a.innerHTML=avatarHTML("ai");a.title="修改 他的状态";a.onclick=e=>{e.stopPropagation();openStatusPicker("ai")};const si=getStatus("ai");const st=document.createElement("span");st.className="msgStatus "+si.cls;st.innerHTML=`<i></i><span>${si.label}</span>`;identity.append(n,a,st);wrap.appendChild(identity);const b=document.createElement("div");b.className="bubble generatedImageBubble";b.title="点击查看大图";const grid=document.createElement("div");grid.className="sentImageGrid generatedImageGrid";const img=document.createElement("img");img.src=src;img.alt="生成的图片";img.loading="lazy";img.addEventListener("click",e=>{e.stopPropagation();openGeneratedImageViewer(src)});grid.appendChild(img);b.appendChild(grid);wrap.appendChild(b);const read=document.createElement("span");read.className="readStatus";read.textContent="已读";wrap.appendChild(read);$("#messages").appendChild(wrap);scroll();if(c&&persist){c.messages.push({role:"assistant",content:"",image:src,timestamp:Date.now()});save();addSharedAlbumImage(src,"刚刚生成的画面")}else if(src){addSharedAlbumImage(src,"生成的画面")} }
...
};$("#momentsUserPost").onclick=()=>{const i=$("#momentsUserText"),v=i.value.trim(),img=$("#momentComposeImagePreview")?.dataset.image||"";if(!v&&!img)return;const post=createMoment("user",v,img);if(post){setAiPresence("刚看到你发的动态",120000);renderMoments();}i.value="";if($("#momentComposerDialog")?.open)$("#momentComposerDialog").close();$("#momentComposeImagePreview").innerHTML="";$("#momentComposeImagePreview").dataset.image="";$("#momentComposeImageFile").value=""};$("#openDrawer").onclick=openDrawer;$("#closeDrawer").onclick=closeDrawer;$("#shade").onclick=closeDrawer;$("#openNest").onclick=openNest;$("#nestCloseHome").onclick=closeNest;$("#nestOpenSettings").onclick=()=>showNestView("settings");$("#nestMoodEdit").onclick=()=>showNestMood("user");$("#nestAiMoodEdit").onclick=()=>showNestMood("ai");const openStory=()=>{showNestView("story")};const openToG=()=>{nestViewDateKey=nestDateKey();showNestView("toG")};const openNote=()=>{nestViewDateKey=nestDateKey();showNestView("note")};const openNotebook=()=>showNestView("notebook");$("#nestToGCard").onclick=openToG;$("#nestNoteCard").onclick=openNote;$("#nestStoryCard").onclick=openStory;$("#nestNotebookCard").onclick=openNotebook;$("#nestToGCard").onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openToG()}};$("#nestNoteCard").onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openNote()}};$("#nestStoryCard").onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openStory()}};$("#nestNotebookCard").onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openNotebook()}};$("#nestToGSave").onclick=()=>{saveToGPage();showNestView("home")};$("#nestAnniversaryOpen").onclick=()=>showNestView("anniversary");document.querySelectorAll("#nest .nestBottomNav button[data-nest-view]").forEach(b=>b.onclick=()=>b.dataset.nestView==="mood"?showNestMood("user"):showNestView(b.dataset.nestView));document.querySelectorAll("#nest [data-nest-back]").forEach(b=>b.onclick=()=>showNestView("home"));$("#nestMoodOptions").onclick=e=>{const b=e.target.closest("[data-mood-index]");if(b)chooseMood(b.dataset.moodIndex)};$("#nestAiMoodOptions").onclick=e=>{const b=e.target.closest("[data-mood-index]");if(b)chooseAiMood(b.dataset.moodIndex)};$("#nestMood").oninput=e=>{saveDailyField(nestViewDateKey,"userMoodNote",e.target.value.trim())};$("#nestAiMood").oninput=e=>{saveDailyField(nestViewDateKey,"aiMoodNote",e.target.value.trim())};$("#nestMoodSave").onclick=()=>{saveMoodPage();renderNestHome();showNestView("home")};$("#nestMoodPrev").onclick=()=>{saveMoodPage();nestViewDateKey=shiftNestDate(nestViewDateKey,-1);renderMoodPage()};$("#nestMoodNext").onclick=()=>{saveMoodPage();nestViewDateKey=shiftNestDate(nestViewDateKey,1);renderMoodPage()};$("#nestAiMoodPrev").onclick=()=>{saveMoodPage();nestViewDateKey=shiftNestDate(nestViewDateKey,-1);renderMoodPage()};$("#nestAiMoodNext").onclick=()=>{saveMoodPage();nestViewDateKey=shiftNestDate(nestViewDateKey,1);renderMoodPage()};$("#nestNoteSave").onclick=()=>{saveNotePage();showNestView("home")};$("#nestNotebookSave").onclick=()=>saveNotebookPage();$("#nestNotebookCancel").onclick=()=>resetNotebookEditor();$("#nestNotebookList").onclick=e=>{const pager=e.target.closest("[data-notebook-page]");if(pager){turnNotebookPage(pager.dataset.notebookPage==="next"?1:-1);return;}const b=e.target.closest("[data-notebook-action]");if(!b)return;const card=b.closest("[data-notebook-id]");if(!card)return;const id=card.dataset.notebookId;if(b.dataset.notebookAction==="edit")editNotebookPage(id);else if(b.dataset.notebookAction==="delete")deleteNotebookPage(id)};$("#nestStoryList").onclick=e=>{const b=e.target.closest("[data-story-action]");if(!b)return;const id=b.dataset.storyId;if(b.dataset.storyAction==="edit")editNestStory(id);else if(b.dataset.storyAction==="delete")deleteNestStory(id)};$("#nestStoryEditSave").onclick=saveNestStoryEdit;$("#nestStoryEditCancel").onclick=resetNestStoryEditor;$("#nestToGPrev").onclick=()=>{saveToGPage();nestViewDateKey=shiftNestDate(nestViewDateKey,-1);loadDailyPages()};$("#nestToGNext").onclick=()=>{saveToGPage();nestViewDateKey=shiftNestDate(nestViewDateKey,1);loadDailyPages()};$("#nestNotePrev").onclick=()=>{saveNotePage();nestViewDateKey=shiftNestDate(nestViewDateKey,-1);loadDailyPages()};$("#nestNoteNext").onclick=()=>{saveNotePage();nestViewDateKey=shiftNestDate(nestViewDateKey,1);loadDailyPages()};$("#nestAnniversarySelect").onchange=e=>selectAnniversary(e.target.value);$("#nestAnniversarySave").onclick=()=>{saveSelectedAnniversary();showNestView("home")};$("#nestAnniversaryNew").onclick=createNewAnniversary;$("#nestAnniversaryDelete").onclick=deleteSelectedAnniversary;$("#nestSettingsSave").onclick=()=>{saveNest();showNestView("home")};$("#nestNotebookOpacity").oninput=e=>{const v=Math.max(45,Math.min(100,Number(e.target.value)||90))/100;nestData.notebookOpacity=v;saveNestData();applyNotebookOpacity()};$("#nestBgPick").onclick=()=>$("#nestBgFile").click();$("#nestBgFile").onchange=async()=>{const f=$("#nestBgFile").files?.[0];if(!f)return;try{nestData.background=await imageToData(f,1400,.78);saveNestData();applyNestBackground()}catch{showErr("小窝背景图片处理失败。")}};$("#nestBgClear").onclick=()=>{nestData.background="";saveNestData();applyNestBackground()};setupAnniversaryBackground();$("#nestSurpriseBtn").onclick=()=>surpriseForUser();$("#nestAlbumOpen").onclick=()=>showNestView("album");$("#nestPetCard")?.addEventListener("click",e=>{if(e.target.closest("[data-pet-action]"))return;showNestView("pet")});$("#nestPetCard")?.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();showNestView("pet")}});$("#nestPetPage")?.addEventListener("click",e=>{const b=e.target.closest("[data-pet-action]");if(b){e.stopPropagation();petAction(b.dataset.petAction)}});$("#nestAlbumPreview")?.addEventListener("click",e=>{if(e.target.closest("[data-album-open]"))showNestView("album")});$("#nestSharedAlbumGrid")?.addEventListener("click",e=>{const item=e.target.closest("[data-album-id]");const b=e.target.closest("[data-album-action]");if(item&&b){const id=item.dataset.albumId;if(b.dataset.albumAction==="edit")editSharedAlbumImage(id);else if(b.dataset.albumAction==="replace")replaceSharedAlbumImage(id);else deleteSharedAlbumImage(id)}});
let momentCropState=null;
function openMomentBgCrop(file){
  const url=URL.createObjectURL(file),img=new Image();
  img.onload=()=>{URL.revokeObjectURL(url);momentCropState={img,scale:1,viewW:0,viewH:0,crop:null,drag:null};const d=$("#momentBgCropDialog");if(!d)return;d.showModal();requestAnimationFrame(renderMomentCrop)};
  img.onerror=()=>{URL.revokeObjectURL(url);showErr("图片读取失败。")};img.src=url;
}
function renderMomentCrop(){
  const st=momentCropState,canvas=$("#momentBgCropCanvas"),box=$("#momentCropBox");if(!st||!canvas||!box)return;
  const maxW=Math.min(innerWidth-24,620),maxH=Math.min(innerHeight*.58,520),scale=Math.min(maxW/st.img.naturalWidth,maxH/st.img.naturalHeight,1);st.scale=scale;st.viewW=Math.round(st.img.naturalWidth*scale);st.viewH=Math.round(st.img.naturalHeight*scale);
  canvas.width=st.viewW;canvas.height=st.viewH;canvas.style.width=st.viewW+"px";canvas.style.height=st.viewH+"px";canvas.getContext("2d").drawImage(st.img,0,0,st.viewW,st.viewH);
  if(!st.crop){const w=st.viewW,h=Math.max(80,Math.min(st.viewH,Math.round(w*.48)));st.crop={x:0,y:Math.max(0,(st.viewH-h)/2),w,h}}
  const c=st.crop,stage=box.parentElement;const ox=Math.max(0,(stage.clientWidth-st.viewW)/2),oy=Math.max(0,(stage.clientHeight-st.viewH)/2);box.style.left=(ox+c.x)+"px";box.style.top=(oy+c.y)+"px";box.style.width=c.w+"px";box.style.height=c.h+"px";
}
function clampMomentCrop(){const st=momentCropState;if(!st?.crop)return;const c=st.crop,min=60;c.w=Math.max(min,Math.min(c.w,st.viewW));c.h=Math.max(min,Math.min(c.h,st.viewH));c.x=Math.max(0,Math.min(c.x,st.viewW-c.w));c.y=Math.max(0,Math.min(c.y,st.viewH-c.h))}
function setupMomentCrop(){
  const box=$("#momentCropBox"),dialog=$("#momentBgCropDialog");if(!box||!dialog||box.dataset.bound)return;box.dataset.bound="1";
  box.addEventListener("pointerdown",e=>{const st=momentCropState;if(!st?.crop)return;e.preventDefault();const h=e.target.closest("[data-crop-handle]");box.setPointerCapture?.(e.pointerId);st.drag={sx:e.clientX,sy:e.clientY,x:st.crop.x,y:st.crop.y,w:st.crop.w,h:st.crop.h,handle:h?.dataset.cropHandle||"move"}});
  box.addEventListener("pointermove",e=>{const st=momentCropState,d=st?.drag;if(!st||!d)return;const dx=e.clientX-d.sx,dy=e.clientY-d.sy,c=st.crop,min=60;if(d.handle==="move"){c.x=d.x+dx;c.y=d.y+dy}else{let nx=d.x,ny=d.y,nw=d.w,nh=d.h;if(d.handle.includes("l")){nx=d.x+dx;nw=d.w-dx}if(d.handle.includes("r"))nw=d.w+dx;if(d.handle.includes("t")){ny=d.y+dy;nh=d.h-dy}if(d.handle.includes("b"))nh=d.h+dy;if(nw<min){if(d.handle.includes("l"))nx=d.x+d.w-min;nw=min}if(nh<min){if(d.handle.includes("t"))ny=d.y+d.h-min;nh=min}c.x=nx;c.y=ny;c.w=nw;c.h=nh}clampMomentCrop();renderMomentCrop()});
  box.addEventListener("pointerup",e=>{if(momentCropState)momentCropState.drag=null;try{box.releasePointerCapture(e.pointerId)}catch{}});
  $("#momentBgCropCancel").onclick=()=>{momentCropState=null;dialog.close()};
  $("#momentBgCropReset").onclick=()=>{if(momentCropState){momentCropState.crop=null;renderMomentCrop()}};
  $("#momentBgCropSave").onclick=()=>{const st=momentCropState;if(!st?.crop)return;const c=st.crop,cv=document.createElement("canvas"),sx=Math.round(c.x/st.scale),sy=Math.round(c.y/st.scale),sw=Math.round(c.w/st.scale),sh=Math.round(c.h/st.scale);cv.width=Math.min(1800,sw);cv.height=Math.min(1000,sh);cv.getContext("2d").drawImage(st.img,sx,sy,sw,sh,0,0,cv.width,cv.height);momentsData.background=cv.toDataURL("image/jpeg",.92);saveMoments();renderMoments();momentCropState=null;dialog.close()};
}
function openMomentEditDialog(post){const d=$("#momentEditDialog");if(!d||!post)return;$("#momentEditText").value=post.content||"";$("#momentEditPreview").innerHTML=post.imageUrl?`<img src="${escapeHtml(post.imageUrl)}" alt="图片预览">`:"";$("#momentEditPreview").dataset.image=post.imageUrl||"";d.dataset.postId=post.id;d.showModal()}
function setupMomentEdit(){const d=$("#momentEditDialog");if(!d||d.dataset.bound)return;d.dataset.bound="1";$("#momentEditCancel").onclick=()=>d.close();$("#momentEditImagePick").onclick=()=>$("#momentEditImageFile").click();$("#momentEditImageFile").onchange=async()=>{const f=$("#momentEditImageFile").files?.[0];if(!f)return;try{const data=await imageToData(f,1200,.84);$("#momentEditPreview").innerHTML=`<img src="${escapeHtml(data)}" alt="图片预览">`;$("#momentEditPreview").dataset.image=data;$("#momentEditImageFile").value=""}catch{showErr("朋友圈图片处理失败。")}};$("#momentEditSave").onclick=()=>{const id=d.dataset.postId,text=$("#momentEditText").value.trim(),img=$("#momentEditPreview").dataset.image||"";if(!text&&!img)return;if(updateUserMoment(id,text,img))d.close()}}
setupMomentCrop();setupMomentEdit();
$("#momentCommentCancel").onclick=()=>$("#momentCommentDialog")?.close();
$("#momentCommentSend").onclick=()=>{const d=$("#momentCommentDialog"),t=$("#momentCommentText"),id=d?.dataset.postId||"",commentId=d?.dataset.commentId||"",text=t?.value.trim()||"";if(!id||!text)return;if(commentId){const cmt=addMomentReply(id,commentId,"user",text);if(cmt)autoAiReplyToUserComment(id,commentId,text)}else{const cmt=addMomentComment(id,"user",text);if(cmt)autoAiReplyToUserComment(id,cmt.id,text)}d.close()};
$("#momentCommentText").oninput=()=>{const t=$("#momentCommentText");if(t)t.value=t.value.slice(0,300)};
$("#momentSignatureCancel").onclick=()=>$("#momentSignatureDialog")?.close();
...
window.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"){setAiPresence("回来啦",90000);setTimeout(()=>{maybeProactiveContact();irisLifeState.lastUserSeen=Date.now();saveIrisLife()},700)}});
