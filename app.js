...
async function handleFiles(files){for(const f of Array.from(files||[])){try{if(f.type.startsWith("image/")){const data=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(String(r.result||""));r.onerror=rej;r.readAsDataURL(f)});state.attachments.push({kind:"image",name:f.name,data})}else if(/^(text\/|application\/(json|xml)|.*\/(javascript|css))/.test(f.type)||/\.(txt|md|json|csv|html|css|js|py|log|xml)$/i.test(f.name)){const text=await f.text();state.attachments.push({kind:"text",name:f.name,text:text.slice(0,30000)})}else{const data=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(String(r.result||""));r.onerror=rej;r.readAsDataURL(f)});state.attachments.push({kind:"file",name:f.name,data})}}catch(e){showErr("读取文件失败："+f.name)}}renderAttachments()}
