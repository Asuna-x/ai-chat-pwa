...
function renderMoodQuickGrid(id,value){const box=$(id);if(!box)return;box.innerHTML="";nestMoodOptions.forEach(label=>{const b=document.createElement("button");b.type="button";b.className="nestMoodQuickOption"+(String(value||"")===label?" active":"");b.textContent=label;b.onclick=()=>{const target=id==="#nestUserMoodQuick"?"#nestMood":"#nestAiMood";$(target).value=label;renderMoodPage()};box.appendChild(b)});const custom=String(value||"")&&!nestMoodOptions.includes(String(value||""));if(custom){const b=document.createElement("span");b.className="nestMoodCustomLabel";b.textContent="自定义："+String(value);box.appendChild(b)}}
function renderMoodPage(){normalizeNestData();const user=nestData.moods.user||"",ai=nestData.moods.ai||"";$("#nestMood").value=user;$("#nestAiMood").value=ai;renderMoodQuickGrid("#nestUserMoodQuick",user);renderMoodQuick("#nestAiMoodQuick",ai)}
