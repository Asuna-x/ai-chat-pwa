...
function appendGeneratedImage(src,c,timestamp=null,chatId=null,messageIndex=null){const b=bubble("assistant","",false,timestamp||Date.now(),false,chatId||c?.id||state.current||"",messageIndex);b.classList.add("generatedImageBubble");b.innerHTML="";const img=document.createElement("img");img.src=src;img.alt="生成的图片";b.appendChild(img);const row=b.closest(".message");if(row)row.classList.add("generatedImageMessage");scroll();if(c){c.messages.push({role:"assistant",content:"[生成图片]",image:src,timestamp:timestamp||Date.now()});save()}}
