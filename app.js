/* Iris v4.83 — vision final reply uses the normal chat model; rate-limit failures stay readable. */
...
async function sendVisionMessage(c,text,attachments){
 ...
 const fail=msg=>{const detail=String(msg||"未知错误");console.error("Iris v4.83 vision",detail);showErr("图片发送失败："+detail);c.messages.push({role:"assistant",content:"图片发送失败："+detail,timestamp:Date.now()});save();render()};
 ...
 try{
  ...
  let vision;
  try{
   vision=await window.GChatAPI.visionChat({baseUrl:visionBase,apiKey:visionKey,model:visionModel,messages:[{role:"user",content}],temperature:.2,signal:controller.signal,max_tokens:1024});
  }catch(err){
   const status=Number(err?.status||0),code=String(err?.code||"");
   if(status===429&&code==="1305"){
    throw new Error("智谱视觉模型当前访问量过大，请稍后再试。这里不会自动重复请求，避免连续触发限流。 ");
   }
   if(status===429){
    const retry=Number(err?.retryAfter||0);
    throw new Error(retry>0?`智谱视觉接口暂时限流，请约 ${Math.ceil(retry)} 秒后再试。` : "智谱视觉接口暂时限流，请稍后再试。 ");
   }
   throw err;
  }
...
async function send(){
 ...
 if(imageAttachments.length){
  ...
  }catch(e){console.error("Iris v4.83 image send",e);showErr("图片发送流程出错："+(e?.message||String(e)))}
  return;
 }
