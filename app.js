...
/* Iris v4.147 — Moments / 朋友圈. Stored separately so it does not touch chat or nest data. */
...
function normalizeMoments(){
  ...
  momentsData.posts=momentsData.posts.filter(x=>x&&typeof x==="object").map(x=>({
    ...
    createdAt:Number(x.createdAt||Date.now()),likes:Math.max(0,Number(x.likes||0)),favorites:Math.max(0,Number(x.favorites||0)),liked:x.liked===true,favorited:x.favorited===true
  })).slice(-100);
}
function saveMoments(){normalizeMoments();localStorage.setItem(momentsStorageKey,JSON.stringify(momentsData))}
normalizeMoments();
function momentsAuthor(post){return post?.role==="user"?(state.settings.myName||"你"):(state.settings.gName||"他")}
...
function renderMoments(){
  ...
  if(name)name.textContent=state.settings.gName||"他";
  if(av)av.innerHTML=avatarHTML("ai");
  ...
  feed.innerHTML=posts.length?posts.map(post=>{
    const likeLabel=post.liked?"已赞":"赞",favLabel=post.favorited?"已收藏":"收藏";
    ...
    return `<article class="momentPost" data-moment-id="${escapeHtml(post.id)}">
      ...
      <div class="momentActions"><button type="button" data-moment-action="like" class="${post.liked?"active":""}">♡ <span>${likeLabel}${post.likes?` ${post.likes}`:""}</span></button><button type="button" data-moment-action="favorite" class="${post.favorited?"active":""}>☆ <span>${favLabel}${post.favorites?` ${post.favorites}`:""}</span></button></div>
    </article>`;
  }).join(""): `<div class="momentsEmpty"><div>○</div><b>这里还没有动态</b><small>他有想分享的东西时，会把它留在这里。</small></div>`;
}
...
function createMoment(role,content,imageUrl=""){
  ...
  const post={id:crypto.randomUUID(),role:role==="user"?"user":"ai",content:text,imageUrl:img,createdAt:Date.now(),likes:0,favorites:0,liked:false,favorited:false};
...
function momentsToolSystem(){return `朋友圈工具规则：这是一个真实的朋友圈功能。你可以在聊天中自然地使用 post_moment 发布一条属于你的朋友圈。用户明确说“发朋友圈/帮你发/你去发”等时可以发布；如果聊天中出现了真正值得你主动分享的一件事、一个瞬间、一个想法，也可以自主发布，不需要每次都等用户下命令。不要为了调用工具而硬发，不要连续刷屏；普通聊天不要自动发。每次最多发布一条。发布内容要像你自己会发的朋友圈，简短、自然、有生活感，不要提工具、API 或内部流程。图片可选；只有确实有合适的图片 URL 时才填写 image_url。`}
...
async function send(){
 ...
 try{
  ...
  let tools=[];if(state.settings.mcpNestEnabled!==false){const nt=nestTools().filter(t=>state.settings.deleteChatBubbleEnabled!==false||!['delete_chat_bubble','delete_chat_bubbles'].includes(t.function?.name));tools.push(...nt);}if(state.settings.mcpEnabled===true&&state.settings.mcpServerUrl){try{const ext=await mcpListTools();tools.push(...ext)}catch(e){console.warn('MCP tool discovery failed',e)}}if(state.settings.imageGenEnabled===true&&state.settings.imageGenBase&&state.settings.imageGenKey&&state.settings.imageGenModel){const gtool=nestTools().find(x=>x.function?.name==="generate_image");if(gtool)tools.push(gtool)}tools=tools.filter((t,i,a)=>a.findIndex(x=>x.function?.name===t.function?.name)===i);if(!tools.length)tools=null;
