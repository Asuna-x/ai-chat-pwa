/* Iris v4.84 — vision fallback chain for Zhipu overloads; normal chat stays on DeepSeek. */
...
/* Iris v4.84 — vision bridge: GLM-4.6V-Flash -> GLM-4.1V-Thinking-Flash -> GLM-4V-Flash on overload. */
...
function visionFallbackModels(base,primary){
 ...
 return [...new Set([p,'glm-4.1v-thinking-flash','glm-4v-flash'])];
}
...
async function sendVisionMessage(c,text,attachments){
 ...
 const fail=msg=>{const detail=String(msg||'未知错误');console.error('Iris v4.84 vision',detail);showErr('图片发送失败：'+detail);c.messages.push({role:'assistant',content:'图片发送失败：\n'+detail,timestamp:Date.now()});save();render()};
 ...
 try{
  ...
  const candidates=visionFallbackModels(visionBase,visionModel);let vision=null,lastVisionError=null,usedVisionModel='';
  ...
  if(!vision)throw lastVisionError||new Error('视觉模型请求失败');
...
async function send(){
 ...
 if(imageAttachments.length){
  ...
  }catch(e){console.error("Iris v4.84 image send",e);showErr("图片发送流程出错："+(e?.message||String(e)))}
  return;
 }
