/* Iris v4.115 — add shared notebook; fix nest footer seam. */
...
function renderNestNotebook(){
 ...
 box.innerHTML=items.length?items.map((x,i)=>`<article class="nestNotebookSheet"><div class="nestNotebookMargin"></div><div class="nestNotebookDate">${escapeHtml(x.date||"")}</div><h3>${escapeHtml(x.title||"这一页，留给我们")}</h3><p>${escapeHtml(x.content||"")}</p><span class="nestNotebookPageNo">${String(items.length-i).padStart(2,"0")}</span></article>`).join(""):`<div class="nestNotebookEmpty"><div class="nestNotebookEmptyIcon">♡</div><b>本子还是空白的</b><small>想留下来的话，就写在这里。</small></div>`;
}
function saveNotebookPage(){
 const content=$("#nestNotebookText")?.value.trim()||""; if(!content){showErr("本子这一页还没有内容。");return;}
 normalizeNestData(); nestData.notebook.push({id:crypto.randomUUID(),date:nestDateKey(),title:($("#nestNotebookTitle")?.value.trim()||"这一页，留给我们").slice(0,60),content,createdAt:Date.now()}); nestData.notebook=nestData.notebook.slice(-80); saveNestData(); renderNestNotebook(); renderNestHome(); $("#nestNotebookTitle").value=""; $("#nestNotebookText").value="";
}
  ...
  while(rounds++<4){let answer="",pendingRound="";const onText=(part,all)=>{answer=all;pendingRound+=part;const parts=splitReply(pendingRound),ready=/[。！？!?；;\n]\s*$/.test(pendingRound),count=ready?parts.length:Math.max(0,parts.length-1);for(let j=0;j<count;j++)pushSentence(parts[j]);pendingRound=count?parts.slice(count).join(""):pendingRound};
   ...
   if(result.toolCalls?.length){ms.push({role:"assistant",content:result.answer||null,tool_calls:result.toolCalls});
    for(const call of result.toolCalls){let args={};try{args=JSON.parse(call.function?.arguments||"{}")}catch{}let out;const callName=call.function?.name||'';try{if(['enter_nest','write_nest_mood','set_nest_ai_mood','write_nest_note','write_nest_to_user','write_nest_notebook','read_nest'].includes(callName))out=executeNestTool(callName,args);else if(callName==='generate_image'){const g=await window.GChatAPI.imageGenerate({baseUrl:state.settings.imageGenBase,apiKey:state.settings.imageGenKey,model:state.settings.imageGenModel,prompt:args.prompt,size:args.size,signal:controller.signal});out={success:true,tool:'generate_image',url:g.url,b64:g.b64};const src=g.url||(g.b64?'data:image/png;base64,'+g.b64:'');if(src)appendGeneratedImage(src,c)}else out=await mcpCallTool(callName,args)}catch(e){out={success:false,error:e?.message||String(e)}}ms.push({role:"tool",tool_call_id:call.id,name:call.function?.name,content:JSON.stringify(out)});
     ...
     if(nestActionTarget&&['write_nest_mood','write_nest_note','write_nest_to_user','write_nest_notebook'].includes(callName)&&out?.success){forcedNestStep=null;}
    }
    continue
   }
