...
function nestDaysText(){const raw=nestData.anniversary;if(!raw)return"还没有设置纪念日";const parts=raw.split("-").map(Number);if(parts.length!==3||parts.some(x=>!Number.isFinite(x)))return"纪念日日期无效";const now=new Date();const today=new Date(now.getFullYear(),now.getMonth(),now.getDate());let next=new Date(today.getFullYear(),parts[1]-1,parts[2]);if(next<today)next=new Date(today.getFullYear()+1,parts[1]-1,parts[2]);const diff=Math.round((next-today)/86400000);return diff===0?"就是今天":`距离下一次纪念日还有 ${diff} 天`}
function updateNestClock(){const n=$("#nestNow");if(n){const d=new Date();n.textContent=d.toLocaleString("zh-TW",{year:"numeric",month:"long",day:"numeric",weekday:"long",hour:"2-digit",minute:"2-digit",hour12:false})}const c=$("#nestCountdown");if(c)c.textContent=nestDaysText()}
function openNest(){nestData=loadNest();$("#nestMood").value=nestData.mood||"";$("#nestToG").value=nestData.toG||"";$("#nestNote").value=nestData.note||"";$("#nestAnniversary").value=nestData.anniversary||"";updateNestClock();clearInterval(nestClockTimer);nestClockTimer=setInterval(updateNestClock,60000);$("#nest").showModal()}
function closeNest(){clearInterval(nestClockTimer);nestClockTimer=null;$("#nest").close()}
function saveNest(){nestData={mood:$("#nestMood").value.trim(),toG:$("#nestToG").value.trim(),note:$("#nestNote").value.trim(),anniversary:$("#nestAnniversary").value||"",updatedAt:Date.now()};saveNestData();updateNestClock();closeNest()}
