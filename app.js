...
function normalizeMoments(){
  ...
  momentsData.posts=momentsData.posts.filter(x=>x&&typeof x==="object").map(x=>({
    ...
    comments:Array.isArray(x.comments)?x.comments.filter(c=>c&&typeof c==="object").map(c=>({id:String(c.id||crypto.randomUUID()),role:c.role==="user"?"user":"ai",content:String(c.content||"").slice(0,300),createdAt:Number(c.createdAt||Date.now())})).slice(-30):[]
  })).slice(-100);
}
...
function renderMoments(){
  ...
  feed.innerHTML=posts.length?posts.map(post=>{
    ...
    const comments=Array.isArray(post.comments)&&post.comments.length?`<div class="momentComments">${post.comments.map(c=>`<div class="momentComment"><b>${escapeHtml(momentsAuthor(c))}</b><span>${escapeHtml(c.content)}</span></div>`).join("")}</div>`:"";
...
function addMomentComment(id,role,content){normalizeMoments();const p=momentsData.posts.find(x=>x.id===id);const text=String(content||"").trim().slice(0,300);if(!p||!text)return false;p.comments=Array.isArray(p.comments)?p.comments:[];p.comments.push({id:crypto.randomUUID(),role:role==="user"?"user":"ai",content:text,createdAt:Date.now()});p.comments=p.comments.slice(-30);saveMoments();renderMoments();return true}
function momentContextForAI(){
  normalizeMoments();
  return momentsData.posts.slice().sort((a,b)=>b.createdAt-a.createdAt).slice(0,10).map(p=>{
    const comments=Array.isArray(p.comments)?p.comments.slice(-8).map(c=>`${momentsAuthor(c)}：${String(c.content||"").slice(0,100)}`).join(" / "):"";
    return `id=${p.id} | ${momentsAuthor(p)} | ${String(p.content||"[图片]").slice(0,160)} | 点赞${Number(p.likes||0)} | 收藏${Number(p.favorites||0)} | 你已赞=${p.aiLiked===true?"是":"否"} | 用户已赞=${p.liked===true?"是":"否"} | 用户已收藏=${p.favorited===true?"是":"否"} | 评论${Array.isArray(p.comments)?p.comments.length:0}${comments?` | 评论内容=${comments}`:""}`;
...
function executePostMoment(args={}){const content=String(args?.content||"").trim();const imageUrl=String(args?.image_url||args?.imageUrl||"").trim();if(!content&&!imageUrl)return{success:false,error:"朋友圈内容为空。"};const post=createMoment("ai",content,imageUrl);return post?{success:true,tool:"post_moment",postId:post.id,author:momentsAuthor(post),createdAt:post.createdAt}: {success:false,error:"朋友圈发布失败。"}}
...
function momentsToolSystem(){const ctx=momentContextForAI();return `朋友圈规则：这是用户视角的朋友圈。用户的头像和名字在顶部，AI发布的动态会出现在同一个动态流里。你可以用 post_moment 发布属于你的动态；用户明确要求时可以发，也可以在聊天中确实有值得分享的事情时自主发，但不要连续刷屏。你也可以用 like_moment 给一条你想回应的动态点赞，用 comment_moment 留下一条自然评论。不要为了调用工具而硬发、硬赞或硬评。\n最近朋友圈：\n${ctx||"暂无动态"}`}
...
function nestTools(){
 return [
  ...
  {type:'function',function:{name:'post_moment',description:'发布一条属于你的朋友圈。用户明确要求时可以发布；如果聊天中有真正值得你主动分享的内容，也可以自然地自主发布，但不要为了调用工具而硬发或连续刷屏。可选附带一张图片 URL。',parameters:{type:'object',properties:{content:{type:'string',description:'朋友圈正文，第一人称，自然简短，像你自己会发的动态'},image_url:{type:'string',description:'可选。要附在朋友圈里的图片 URL；没有合适图片时留空'}},required:['content']}}},
...
$("#momentsBgFile").onchange=async()=>{const f=$("#momentsBgFile").files?.[0];if(!f)return;try{openMomentBgCrop(f);$("#momentsBgFile").value=""}catch{showErr("朋友圈背景图片处理失败。")}};$("#momentComposeImagePick").onclick=()=>$("#momentComposeImageFile").click();$("#momentComposeImageFile").onchange=async()=>{const f=$("#momentComposeImageFile").files?.[0];if(!f)return;try{const data=await imageToData(f,1200,.84);$("#momentComposeImagePreview").innerHTML=`<img src="${data}" alt="预览">`;$("#momentComposeImagePreview").dataset.image=data}catch{showErr("朋友圈图片处理失败。")}};$("#momentsFeed").onclick=e=>{
  ...
  if(action==="comment"){const text=prompt("评论这条动态");if(text)addMomentComment(id,"user",text);return}
