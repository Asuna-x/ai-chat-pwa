/* Iris v4.80 — vision transport rebuilt from scratch; text send path preserved. */
...
async function sendVisionMessage(c,text,attachments){
 ...
 const fail=msg=>{const detail=String(msg||"未知错误");console.error("Iris v4.80 vision",detail);showErr("图片发送失败："+detail);c.messages.push({role:"assistant",content:"图片发送失败：\n"+detail,timestamp:Date.now()});save();render()};
