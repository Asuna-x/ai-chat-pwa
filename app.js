...
function appendGeneratedImage(src,c,persist=true,messageIndex=null){const wrap=document.createElement("div");wrap.className="message assistant generatedImageMessage";wrap.dataset.chatId=c?.id||state.current||"";if(messageIndex!==null)wrap.dataset.messageIndex=String(messageIndex);const identity=document.createElement("div");identity.className="msgIdentity";const n=document.createElement("span");n.className="msgName";n.textContent=state.settings.gName||"他";const a=document.createElement("div");a.className="msgavatar statusClickable";a.innerHTML=avatarHTML("ai");const si=getStatus("ai");const st=document.createElement("span");st.className="msgStatus "+si.cls;st.innerHTML=`<i></i><span>${si.label}</span>`;identity.append(n,a,st);wrap.appendChild(identity);const bubble=document.createElement("div");bubble.className="bubble generatedImageBubble";bubble.title="点击查看大图";const img=document.createElement("img");img.src=src;img.alt="生成的图片";img.loading="lazy";img.addEventListener("click",e=>{e.stopPropagation();openGeneratedImageViewer(src)});bubble.appendChild(img);wrap.appendChild(bubble);const read=document.createElement("span");read.className="readStatus";read.textContent="已读";wrap.appendChild(read);$("#messages").appendChild(wrap);scroll();if(c&&persist){c.messages.push({role:"assistant",content:"",image:src,timestamp:Date.now()});save()}}
