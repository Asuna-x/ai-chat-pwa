...
async function sendVisionMessage(c,text,attachments){
 ...
 try{
  ...
  let vision,lastVisionError;for(let attempt=0;attempt<3;attempt++){try{vision=await window.GChatAPI.visionChat({baseUrl:visionBase,apiKey:visionKey,model:visionModel,messages:[{role:"user",content}],temperature:.2,signal:controller.signal,max_tokens:1024});break}catch(err){lastVisionError=err;const msg=String(err?.message||err);if(!/HTTP 429\b|code.?[:=]?\s*["\']?1305/i.test(msg)||attempt===2)throw err;await new Promise(resolve=>setTimeout(resolve,1500*(attempt+1)))}}if(!vision&&lastVisionError)throw lastVisionError;const visualAnswer=String(vision.answer||"").trim();if(!visualAnswer)throw new Error("视觉模型返回为空");
