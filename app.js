...
function nestTools(){
 return [
  ...
  {type:'function',function:{name:'delete_chat_bubble',description:'删除当前聊天中的一条聊天气泡。主要用于刚才生成图片时出现重复图片或明显多余的图片气泡。只删除 AI 生成的图片，不删除用户上传的图片。优先使用 target=duplicate_generated_image：当最近有重复生成图片时，删除较早的那一张，保留最新的一张。也可使用 target=last_generated_image 删除最近一张生成图片。只有确实需要删除时才调用。',parameters:{type:'object',properties:{target:{type:'string',enum:['duplicate_generated_image','last_generated_image'],description:'删除目标：duplicate_generated_image 删除最近两张生成图片中较早的一张；last_generated_image 删除最近一张生成图片'},message_index:{type:'integer',description:'可选。明确知道消息索引时，直接删除该索引对应的 AI 生成图片消息。'}},required:['target']}}}
 ];
}
...
function executeNestTool(name,args){
 ...
 if(name==='delete_chat_bubble')return executeDeleteChatBubble(args);
