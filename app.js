...
function renderChatList(){const l=$("#chatList");if(!l)return;l.innerHTML="";const now=new Date();const today=new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime();const yesterday=today-86400000;const groups={};state.chats.forEach(x=>{const t=x.createdAt||0;const key=t>=today?"今天":t>=yesterday?"昨天":"更早";(groups[key]??=[]).push(x)});["今天","昨天","更早"].forEach(key=>{if(!groups[key]?.length)return;const g=document.createElement("div");g.className="chatGroup";g.innerHTML=`<div class="chatGroupTitle">${key}</div>`;groups[key].forEach(x=>{const row=document.createElement("div");row.className="itemRow"+(state.chatSelectMode&&state.selectedChats.has(x.id)?" selected":"");const b=document.createElement("button");b.className="item"+(x.id===state.current&&!state.chatSelectMode?" active":"");b.innerHTML=`<span class="chatSelectCheck ${state.chatSelectMode&&state.selectedChats.has(x.id)?"checked":""}"></span><span class="itemTitle">${escapeHtml(x.title||"新对话")}</span><span class="itemTime">${formatTime(x.createdAt)}</span>`;let timer=null;b.onclick=()=>{if(state.ignoreNextChatClick){state.ignoreNextChatClick=false;return}if(state.chatSelectMode){toggleChatSelection(x.id);return}state.current=x.id;save();render();closeDrawer()};b.addEventListener("pointerdown",()=>{clearTimeout(timer);if(!state.chatSelectMode)timer=setTimeout(()=>{enterChatSelectMode(x.id)},550)});["pointerup","pointercancel","pointerleave"].forEach(ev=>b.addEventListener(ev,()=>clearTimeout(timer)));b.addEventListener("contextmenu",e=>{e.preventDefault();if(!state.chatSelectMode)enterChatSelectMode(x.id)});row.append(b);g.appendChild(row)});l.appendChild(g)});const count=$("#chatCount");if(count)count.textContent=state.chats.length?state.chats.length+" 个":"";updateChatSelectionUI()}
...
function updateChatSelectionUI(){const bar=$("#chatSelectBar"),newBtn=$("#newChat"),count=$("#selectedChatCount");if(!bar)return;bar.classList.toggle("hidden",!state.chatSelectMode);if(newBtn)newBtn.classList.toggle("hidden",state.chatSelectMode);if(count)count.textContent=`已选择 ${state.selectedChats.size} 个`;const rn=$("#renameSelected");if(rn)rn.disabled=state.selectedChats.size!==1;const del=$("#deleteSelected");if(del)del.disabled=!state.selectedChats.size}
...
function settings(){ensureMigrationUI();fillSettings();renderTokenStats();settingsGoHome();$("#settings").showModal()}
...
function ensureMigrationUI(){
 const page=$("#chatPage");
 if(!page||$("#migrationBox"))return;
 const box=document.createElement("div");
 box.id="migrationBox";
 box.className="settingsGroup";
 box.innerHTML=`<div class="settingsLabel">设备数据继承</div><p style="margin:6px 0 12px;color:var(--muted);font-size:13px;line-height:1.5">用于 Safari 和主屏幕 App 之间迁移聊天、头像、主题、API 设置等本地数据。数据只在你设备之间手动传递。</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" id="makeMigration" class="settingsBtn">生成迁移码</button><button type="button" id="loadMigration" class="settingsBtn">导入迁移码</button></div><textarea id="migrationCode" placeholder="生成后迁移码会显示在这里；在另一台/另一环境打开后粘贴导入。" style="width:100%;min-height:110px;margin-top:10px;resize:vertical"></textarea><p style="margin:8px 0 0;color:var(--muted);font-size:12px;line-height:1.45">注意：迁移码包含 API Key 和你的本地数据，请不要发给别人。</p>`;
 page.appendChild(box);
 $("#makeMigration").onclick=async()=>{
   const ta=$("#migrationCode");ta.value=encodeMigrationData();
   try{await navigator.clipboard.writeText(ta.value);showErr("迁移码已生成并复制。请在另一环境导入。")}
   catch{ta.focus();ta.select();showErr("迁移码已生成，请手动复制。")}
 };
 $("#loadMigration").onclick=()=>{
   const ta=$("#migrationCode"),code=ta.value.trim();
   if(!code){showErr("请先粘贴迁移码。");return}
   try{
     const data=decodeMigrationData(code);
     if(!confirm("导入会覆盖当前聊天记录和设置，确定继续吗？"))return;
     state.chats=data.chats;state.current=data.current||state.chats[0]?.id||null;state.settings=data.settings||{};
     ensure();ensureDates();save();render();fillSettings();
     ta.value="";showErr("数据已成功继承。")
   }catch(e){showErr(e.message||"导入迁移码失败。")}
 };
}
async function importAll(file){try{const text=await file.text(),data=JSON.parse(text);if(!data||!Array.isArray(data.chats)||typeof data.settings!=="object")throw new Error("备份文件格式不正确");if(!confirm("恢复备份会覆盖当前聊天记录和设置，确定继续吗？"))return;state.chats=data.chats;state.current=data.current||state.chats[0]?.id||null;state.settings=data.settings||{};ensure();ensureDates();save();render();fillSettings();showErr("备份已恢复")}catch(e){showErr(e.message||"恢复备份失败")}}
