...
async function sendVisionMessage(c,text,attachments){
 const visionProxy=String(state.settings.visionProxy||"").trim(),visionBase=String(state.settings.visionBase||state.settings.apiBase||"").trim(),visionKey=String(state.settings.visionKey||state.settings.apiKey||"").trim(),visionModel=String(state.settings.visionModel||state.settings.model||"").trim();
 ...
 if((!visionProxy&&(!visionBase||!visionKey))||!visionModel){fail(visionProxy?"Vision 代理配置不完整。请检查代理地址和视觉模型。":"视觉配置不完整。请检查 Vision Base URL、API Key 和视觉模型。");return false}
 ...
 try{
  ...
  const vision=await window.GChatAPI.visionChat({proxyUrl:visionProxy,baseUrl:visionBase,apiKey:visionKey,model:visionModel,messages:[{role:"user",content}],temperature:.2,signal:controller.signal,max_tokens:1024});const visualAnswer=String(vision.answer||"").trim();if(!visualAnswer)throw new Error("视觉模型返回为空");
