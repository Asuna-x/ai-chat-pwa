/* Iris v4.119 — tighten nest footer, refine home clock spacing, soften drawer, and improve notebook lines. */
...
function turnNotebookPage(delta){const items=notebookItems();if(!items.length)return;const next=nestNotebookPageIndex+delta;if(next<0||next>=items.length)return;nestNotebookPageIndex=next;const page=$("#nestNotebookList .nestBookPage");if(page){page.classList.add(delta>0?"turn-next":"turn-prev");setTimeout(()=>renderNestNotebook(),180)}else renderNestNotebook();}
