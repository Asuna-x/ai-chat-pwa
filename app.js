...
function renderSharedAlbum(){normalizeNestData();const a=Array.isArray(nestData.sharedAlbum)?nestData.sharedAlbum:[];const box=$("#nestSharedAlbumGrid");if(box)box.innerHTML=a.length?a.map(x=>`<figure class="nestAlbumItem"><img src="${escapeHtml(x.src)}" loading="lazy" alt="共同相册"><figcaption>${escapeHtml(x.caption||"")}</figcaption></figure>`).join(""): '<div class="nestAlbumEmpty">还没有一起收藏的画面。</div>';const home=$("#nestSharedAlbumHomeGrid");if(home){const preview=a.slice(0,4);home.innerHTML=preview.length?preview.map(x=>`<img src="${escapeHtml(x.src)}" loading="lazy" alt="共同相册里的画面">`).join(""): '<div class="nestAlbumHomeEmpty">还没有一起收藏的画面。生成的图片会慢慢留在这里。</div>'}}
...
function renderPet(){normalizeNestData();const pet=nestData.pet||{};const name=$("#nestPetName"),mood=$("#nestPetMood"),hunger=$("#nestPetHunger"),energy=$("#nestPetEnergy"),level=$("#nestPetLevel"),face=$("#nestPetFace"),note=$("#nestPetNote"),moodBar=$("#nestPetMoodBar");if(name)name.textContent=pet.name||"小家伙";if(mood)mood.textContent=pet.mood||"期待";if(hunger)hunger.style.width=Math.round(pet.hunger||0)+"%";if(energy)energy.style.width=Math.round(pet.energy||0)+"%";if(level)level.textContent="Lv."+(pet.level||1);if(face)face.textContent=pet.mood==="难过"?"(·︵·)":pet.mood==="满足"?"(｡•ᴗ•｡)":"(｡･ω･｡)";if(moodBar)moodBar.style.width=Math.round(Math.max(0,Math.min(100,(nestMoodOptions.indexOf(pet.mood)+1)*12.5)))+"%";if(note)note.textContent=pet.lastInteraction?"刚刚陪过它。":"它一直在小窝里等着你。"}
