/* Iris v4.121 — refine nest clock/save layout and add story edit/delete controls. */
...
function turnNotebookPage(delta){const items=notebookItems();if(!items.length)return;const next=nestNotebookPageIndex+delta;if(next<0||next>=items.length)return;nestNotebookPageIndex=next;renderNestNotebook();}
...
function applyMemoryMaintenance(data){
 ...
 if(Array.isArray(data.episodes)){for(const e of data.episodes.slice(0,5)){const title=String(e?.title||'').trim(),summary=String(e?.summary||'').trim();if(!title||!summary)continue;const key=(title+'|'+summary).replace(/\s+/g,'').slice(0,500);let old=state.memoryEpisodes.find(x=>x.key===key||((String(x.title||'').replace(/\s+/g,'')===title)&&(String(x.summary||'').replace(/\s+/g,'')===summary)));if(!old){state.memoryEpisodes.unshift({id:crypto.randomUUID(),key,title,summary,date:String(e.date||'').trim(),importance:Math.max(1,Math.min(5,Number(e.importance||3))),createdAt:Date.now()});changed=true}}}
