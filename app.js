/* Iris v4.82 — vision transport rebuilt from scratch; text send path preserved. */
...
const IMAGE_DB_NAME="iris-images-v4",IMAGE_DB_STORE="images";
let imageDbPromise=null;
function openImageDB(){if(!window.indexedDB)return Promise.reject(new Error("当前浏览器不支持图片本地存储"));if(imageDbPromise)return imageDbPromise;imageDbPromise=new Promise((resolve,reject)=>{const r=indexedDB.open(IMAGE_DB_NAME,1);r.onupgradeneeded=()=>{if(!r.result.objectStoreNames.contains(IMAGE_DB_STORE))r.result.createObjectStore(IMAGE_DB_STORE)};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error||new Error("图片数据库打开失败"))});return imageDbPromise}
function imageDbPut(data,id=crypto.randomUUID()){return openImageDB().then(db=>new Promise((resolve,reject)=>{const tx=db.transaction(IMAGE_DB_STORE,"readwrite"),st=tx.objectStore(IMAGE_DB_STORE);st.put(String(data),id);tx.oncomplete=()=>resolve(id);tx.onerror=()=>reject(tx.error||new Error("图片保存失败"))}))}
function imageDbGet(id){return openImageDB().then(db=>new Promise((resolve,reject)=>{const r=db.transaction(IMAGE_DB_STORE,"readonly").objectStore(IMAGE_DB_STORE).get(id);r.onsuccess=()=>resolve(typeof r.result==="string"?r.result:"");r.onerror=()=>reject(r.error||new Error("图片读取失败"))}))}
function isImageRef(u){return typeof u==="string"&&u.startsWith("iris-idb://")}
function imageRefId(u){return isImageRef(u)?u.slice("iris-idb://".length):""}
function imageUrlForDisplay(u,img){if(!isImageRef(u)){img.src=u;return}imageDbGet(imageRefId(u)).then(data=>{if(data)img.src=data}).catch(()=>{})}
async function persistMessageImages(message){if(!message||!Array.isArray(message.content))return;for(const x of message.content){const u=x?.type==="image_url"?x.image_url?.url:"";if(typeof u==="string"&&u.startsWith("data:image/")){const id=await imageDbPut(u);x.image_url={url:"iris-idb://"+id}}}}
async function persistChatImages(c){if(!c?.messages)return;for(const m of c.messages)await persistMessageImages(m)}
const save=()=>{try{localStorage.setItem("gchat_chats",JSON.stringify(state.chats));localStorage.setItem("gchat_current",state.current||"");localStorage.setItem("gchat_settings",JSON.stringify(state.settings));localStorage.setItem("gchat_memories",JSON.stringify(state.memories||[]))}catch(e){console.warn("Iris v4.82 localStorage save",e);showErr("聊天文字已保存，但图片改用本地图片存储。")}};
...
function render(){applyLook();const c=chat();$("#title").textContent=c?.title||"新对话";$("#modelName").textContent=state.settings.model||"未設定模型";updateHeaderStatus();$("#headerTime").textContent=c?.messages?.length?" · "+formatTime(c.messages[c.messages.length-1].timestamp):"";updateTyping();const topAvatar=$("#headAvatar"),topMode=state.settings.topAvatar||"user";topAvatar.classList.toggle("is-hidden",topMode==="none");$("#openProfile").classList.toggle("avatar-hidden",topMode==="none");if(topMode!=="none")topAvatar.innerHTML=avatarHTML(topMode);$("#brandAvatar").innerHTML=avatarHTML("ai");renderChatList();const box=$("#messages");box.innerHTML="";if(!c||!c.messages.length){box.innerHTML=`<div class="empty"><div><div class="avatar">${avatarHTML("ai")}</div><span class="welcomeKicker">${welcomeText()}</span><h1>${escapeHtml(state.settings.gName||"他")}</h1><p>${escapeHtml(state.settings.gBio||"你的私人 AI 对话空间")}</p><div class="quickPrompts"><button data-prompt="帮我规划一下今天的安排">规划今天</button><button data-prompt="陪我聊聊，轻松一点">陪我聊聊</button><button data-prompt="帮我整理一下最近的想法">整理思绪</button></div></div></div>`;box.querySelectorAll("[data-prompt]").forEach(b=>b.onclick=()=>{iFill(b.dataset.prompt)});return}c.messages.forEach((m,idx)=>{if(m.image)appendGeneratedImage(m.image,c);else bubble(m.role,m.content,false,m.timestamp,true)});scroll()}
function displayMessageText(value){if(Array.isArray(value))return value.filter(x=>x?.type==="text").map(x=>x.text||"").join("\n");return String(value??"")}
function bubble(role,text,streaming=false,timestamp=null,read=false){const originalContent=text;text=displayMessageText(text);const r=document.createElement("div");r.className="message "+role;if(streaming)r.style.animation="none";const tm=timestamp?document.createElement("span"):null;if(tm){tm.className="msgTime";tm.textContent=formatTime(timestamp)}const type=role==="assistant"?"ai":"user";const name=role==="assistant"?(state.settings.gName||"他"):(state.settings.myName||"你");const makeAvatar=type=>{const wrap=document.createElement("div");wrap.className="msgIdentity";const n=document.createElement("span");n.className="msgName";n.textContent=type==="ai"?(state.settings.gName||"他"):(state.settings.myName||"你");const a=document.createElement("div");a.className="msgavatar statusClickable";a.innerHTML=avatarHTML(type);a.title=type==="ai"?"修改 他的状态":"修改你的状态";a.onclick=e=>{e.stopPropagation();openStatusPicker(type)};const si=getStatus(type);const st=document.createElement("span");st.className="msgStatus "+si.cls;st.innerHTML=`<i></i><span>${si.label}</span>`;wrap.append(n,a,st);return wrap};const makeRead=()=>{const st=document.createElement("span");st.className="readStatus";st.textContent="已读";return st};if(role==="assistant"){r.appendChild(makeAvatar("ai"));const b=document.createElement("div");b.className="bubble";b.textContent=text;b.dataset.text=text;r.appendChild(b);if(read)r.appendChild(makeRead());if(tm)r.appendChild(tm);$("#messages").appendChild(r);bindLongPress(b);return b}if(tm)r.appendChild(tm);const b=document.createElement("div");b.className="bubble";b.textContent=text;b.dataset.text=text;if(Array.isArray(originalContent)){const imgs=originalContent.filter(x=>x?.type==="image_url"&&x?.image_url?.url);if(imgs.length){const wrap=document.createElement("div");wrap.className="sentImageGrid";imgs.forEach(x=>{const img=document.createElement("img");img.alt="已发送图片";imageUrlForDisplay(x.image_url.url,img);wrap.appendChild(img)});b.appendChild(wrap)}}r.appendChild(b);if(read)r.appendChild(makeRead());r.appendChild(makeAvatar("user"));$("#messages").appendChild(r);bindLongPress(b);return b}
...
/* Iris v4.82 — minimal vision request: no chat history, no extra system prompt, no detail field. */
async function sendVisionMessage(c,text,attachments,overrideContent){
 ...
 const fail=msg=>{const detail=String(msg||"未知错误");console.error("Iris v4.82 vision",detail);showErr("图片发送失败："+detail);c.messages.push({role:"assistant",content:"图片发送失败：\n"+detail,timestamp:Date.now()});save();render()};
 ...
 try{
  ...
  c.messages.push({role:"assistant",content:answer,timestamp:Date.now()});save();render();return true;
...
async function recentChatImages(c){
 const out=[];
 for(let n=(c?.messages?.length||0)-1;n>=0&&out.length<4;n--){
  ...
  for(const x of m.content){
   ...
   else if(isImageRef(u)){const data=await imageDbGet(imageRefId(u)).catch(()=>"");if(data)out.push(data)}
  }
 }
 return out;
}
...
async function send(){
 ...
 if(imageAttachments.length){
  try{
   ...
   /* v4.82: image bytes live in IndexedDB; chat JSON keeps only tiny references. */
   await persistMessageImages(userMessage);
  ...
  }catch(e){console.error("Iris v4.82 image bubble",e);showErr("图片加入聊天失败："+(e?.message||String(e)))}
  return;
 }
 ...
 const recentImages=await recentChatImages(c);
...
(async()=>{try{for(const c of state.chats)await persistChatImages(c);save();render()}catch(e){console.warn("Iris v4.82 image migration",e)}})();
