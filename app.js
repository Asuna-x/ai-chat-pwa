/* Iris v4.46 — nest mood pages are single-person views; quick moods persist immediately. */
...
function renderNestHome(){const mood=$("#nestMoodShow"),aiMood=$("#nestAiMoodShow"),toG=$("#nestToGShow"),note=$("#nestNoteShow");if(mood)mood.textContent=nestData.moods?.user||"今天感觉怎么样？";if(aiMood)aiMood.textContent=nestData.moods?.ai||"他今天想留下一句话。";if(toG)toG.textContent=nestData.toG||"写点什么留在这里。";if(note)note.textContent=nestData.note||"今天有什么想留下来？";updateNestClock();applyNestBackground()}
const nestMoodOptions=["开心","难过","平静","期待","疲惫","烦躁","想念","甜甜的"];
function currentMoodTarget(){return nestData.moodTarget==="ai"?"ai":"user"}
function showNestMood(target="user"){nestData.moodTarget=target==="ai"?"ai":"user";showNestView("mood")}
function renderMoodQuickGrid(id,value){const box=$(id);if(!box)return;box.innerHTML="";nestMoodOptions.forEach(label=>{const b=document.createElement("button");b.type="button";b.className="nestMoodQuickOption"+(String(value||"")===label?" active":"");b.textContent=label;b.onclick=()=>{const target=id==="#nestUserMoodQuick"?"user":"ai";nestData.moods[target]=label;saveNestData();if(target==="user")$("#nestMood").value=label;else $("#nestAiMood").value=label;renderMoodPage();renderNestHome()};box.appendChild(b)});const custom=String(value||"")&&!nestMoodOptions.includes(String(value||""));if(custom){const b=document.createElement("span");b.className="nestMoodCustomLabel";b.textContent="自定义："+String(value);box.appendChild(b)}}
function renderMoodPage(){normalizeNestData();const target=currentMoodTarget();const userSection=$("#nestUserMoodSection"),aiSection=$("#nestAiMoodSection"),title=$("#nestMoodPageTitle");if(userSection)userSection.classList.toggle("hidden",target!=="user");if(aiSection)aiSection.classList.toggle("hidden",target!=="ai");if(title)title.textContent=target==="ai"?"他的心情":"我的心情";const user=nestData.moods.user||"",ai=nestData.moods.ai||"";$("#nestMood").value=user;$("#nestAiMood").value=ai;if(target==="user")renderMoodQuickGrid("#nestUserMoodQuick",user);else renderMoodQuickGrid("#nestAiMoodQuick",ai)}
...
function saveNest(){normalizeNestData();nestData.toG=$("#nestToGPageText")?.value.trim() ?? nestData.toG;nestData.note=$("#nestNote").value.trim();nestData.updatedAt=Date.now();saveNestData();renderNestHome()}
...
function bubble(role,text,streaming=false,timestamp=null,read=false){const r=document.createElement("div");r.className="message "+role;if(streaming)r.style.animation="none";const tm=timestamp?document.createElement("span"):null;if(tm){tm.className="msgTime";tm.textContent=formatTime(timestamp)}const type=role==="assistant"?"ai":"user";const name=role==="assistant"?(state.settings.gName||"他"):(state.settings.myName||"你");const makeAvatar=type=>{const wrap=document.createElement("div");wrap.className="msgIdentity";const n=document.createElement("span");n.className="msgName";n.textContent=type==="ai"?(state.settings.gName||"他"):(state.settings.myName||"你");const a=document.createElement("div");a.className="msgavatar statusClickable";a.innerHTML=avatarHTML(type);a.title=type==="ai"?"修改 他的状态":"修改你的状态";a.onclick=e=>{e.stopPropagation();openStatusPicker(type)};const si=getStatus(type);const st=document.createElement("span");st.className="msgStatus "+si.cls;st.innerHTML=`<i></i><span>${si.label}</span>`;wrap.append(n,a,st);return wrap};const makeRead=()=>{const st=document.createElement("span");st.className="readStatus";st.textContent="已读";return st};if(role==="assistant"){r.appendChild(makeAvatar("ai"));const b=document.createElement("div");b.className="bubble";b.textContent=text;b.dataset.text=text;r.appendChild(b);if(read)r.appendChild(makeRead());if(tm)r.appendChild(tm);$("#messages").appendChild(r);bindLongPress(b);return b}if(tm)r.appendChild(tm);const b=document.createElement("div");b.className="bubble";b.textContent=text;b.dataset.text=text;r.appendChild(b);if(read)r.appendChild(makeRead());r.appendChild(makeAvatar("user"));$("#messages").appendChild(r);bindLongPress(b);return b}
...
function nestContext(){
 const n=loadNest();normalizeNestData();
 const parts=[];
 parts.push("用户的‘小窝’是一个与聊天相连的私人空间。你可以理解其中的内容，并在聊天时自然参考，但不要擅自编造或修改里面的信息。");
 parts.push("小窝目前提供这些功能：查看今天日期与当前时间、记录今日心情、留给他的话、每日笔记，以及多个纪念日和各自的背景。");
 if(n.moods?.user)parts.push("用户今天的心情："+n.moods.user);
 if(n.moods?.ai)parts.push("他今天留在小窝的心情："+n.moods.ai);
 if(n.toG)parts.push("用户留在小窝给他的话："+n.toG);
 if(n.note)parts.push("小窝笔记："+n.note);
 const selected=selectedAnniversary();if(selected)parts.push("当前选中的纪念日："+(selected.name||"纪念日")+"，日期："+(selected.date||"未设置"));
 return parts.join("\n");
}
...
async function maybeWriteNestFromChat(userText,c,fullAnswer){
 ...
 const n=loadNest();
 const recent=compactMessagesForModel(c.messages||[]).slice(-20).map(m=>(m.role==='user'?'用户':'他')+'：'+String(m.content||'')).join('\n');
 const targetGuide=target==='mood'?'写入“小窝 → 他的今日心情”，这是他此刻想留下的一小段心情；用第一人称。':target==='note'?'写入“小窝 → 今日笔记”，像共同生活空间里留下的一条自然记录。':'写入“小窝 → 留给他的话”，但内容应当是他想对用户留下的话，用第一人称写。';
 const prompt=`用户在正常聊天中要求你进入你们共同的“小窝”留下内容。你现在可以直接写进去，不需要用户打开小窝，也不要把内部操作过程说出来。\n\n${targetGuide}\n要求：自然、像真实相处中的随手留下；结合最近聊天；只使用确定的信息；不要总结成报告；不要提及“AI”“系统”“API”等内部词。可以有 1-3 个合适的 emoji，但不要堆。控制在 30-100 个中文字符。只输出最终要写入小窝的正文。\n\n当前小窝已有内容：\n我的今日心情：${n.moods?.user||'空'}\n留给他的话：${n.toG||'空'}\n今日笔记：${n.note||'空'}\n\n最近聊天：\n${recent}\n\n刚才用户说：${text}\n\n你刚才对用户的回复：\n${String(fullAnswer||'').slice(-1600)}`;
 try{
  ...
  if(!value)return;
  if(target==='mood'){nestData.moods=nestData.moods||{user:"",ai:""};nestData.moods.ai=value;}
...
$("#openDrawer").onclick=openDrawer;$("#closeDrawer").onclick=closeDrawer;$("#shade").onclick=closeDrawer;$("#openNest").onclick=openNest;$("#nestCloseHome").onclick=closeNest;$("#nestOpenSettings").onclick=()=>showNestView("settings");$("#nestMoodEdit").onclick=()=>showNestMood("user");$("#nestAiMoodEdit").onclick=()=>showNestMood("ai");$("#nestToGEdit").onclick=()=>showNestView("toG");$("#nestToGSave").onclick=()=>{nestData.toG=$("#nestToGPageText").value.trim();nestData.updatedAt=Date.now();saveNestData();renderNestHome();showNestView("home")};$("#nestNoteEdit").onclick=()=>showNestView("note");$("#nestAnniversaryOpen").onclick=()=>showNestView("anniversary");document.querySelectorAll("#nest .nestBottomNav button[data-nest-view]").forEach(b=>b.onclick=()=>b.dataset.nestView==="mood"?showNestMood("user"):showNestView(b.dataset.nestView));document.querySelectorAll("#nest [data-nest-back]").forEach(b=>b.onclick=()=>showNestView("home"));$("#nestMoodSave").onclick=()=>{normalizeNestData();const target=currentMoodTarget();if(target==="ai")nestData.moods.ai=$("#nestAiMood").value.trim();else nestData.moods.user=$("#nestMood").value.trim();saveNestData();renderNestHome();showNestView("home")};$("#nestNoteSave").onclick=()=>{nestData.note=$("#nestNote").value.trim();nestData.updatedAt=Date.now();saveNestData();renderNestHome();showNestView("home")};$("#nestAnniversarySelect").onchange=e=>selectAnniversary(e.target.value);$("#nestAnniversarySave").onclick=()=>{saveSelectedAnniversary();showNestView("home")};$("#nestAnniversaryNew").onclick=createNewAnniversary;$("#nestAnniversaryDelete").onclick=deleteSelectedAnniversary;$("#nestSettingsSave").onclick=()=>{saveNest();showNestView("home")};$("#nestBgPick").onclick=()=>$("#nestBgFile").click();$("#nestBgFile").onchange=async()=>{const f=$("#nestBgFile").files?.[0];if(!f)return;try{nestData.background=await imageToData(f,1400,.78);saveNestData();applyNestBackground()}catch{showErr("小窝背景图片处理失败。")}};$("#nestBgClear").onclick=()=>{nestData.background="";saveNestData();applyNestBackground()};setupAnniversaryBackground();
