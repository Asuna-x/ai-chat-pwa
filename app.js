...
function storyWithChatNames(value){
 ...
 return text.replace(/用户/g,myName).replace(/AI/g,gName).replace(/助手/g,gName);
}
function renderNestStory(){
 const box=$("#nestStoryList"); if(!box)return;
 const eps=(state.memoryEpisodes||[]).slice().sort((a,b)=>(String(b.date||"").localeCompare(String(a.date||"")))||(Number(b.createdAt||0)-Number(a.createdAt||0))).slice(0,40);
