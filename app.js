...
function renderNestHome(){syncTodayToDaily();const e=dailyEntry(),mood=$("#nestMoodShow"),aiMood=$("#nestAiMoodShow"),toG=$("#nestToGShow"),note=$("#nestNoteShow"),ua=$("#nestUserAvatar"),ga=$("#nestAiAvatar"),un=$("#nestUserName"),gn=$("#nestAiName");if(mood)mood.textContent=e.userMood||"今天感觉怎么样？";if(aiMood)aiMood.textContent=e.aiMood||"他今天想留下一句话。";if(toG)toG.textContent=e.toG||"写点什么留在这里。";if(note)note.textContent=e.note||"今天有什么想留下来？";if(ua)ua.innerHTML=avatarHTML("user");if(ga)ga.innerHTML=avatarHTML("ai");if(un)un.textContent=state.settings.myName||"你";if(gn)gn.textContent=state.settings.gName||"他";updateNestClock();applyNestBackground()}
