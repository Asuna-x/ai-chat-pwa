...
function momentContextForAI(){normalizeMoments();return momentsData.posts.slice().sort((a,b)=>b.createdAt-a.createdAt).slice(0,6).map(p=>`id=${p.id} | ${momentsAuthor(p)} | ${String(p.content||"[图片]").slice(0,160)} | ${p.aiLiked?"已赞":"未赞"}`).join("\n")}
