...
function renderMemories(){
 ...
 const eps=(state.memoryEpisodes||[]).slice().sort((a,b)=>(Number(b.importance||0)-Number(a.importance||0))||(Number(b.createdAt||0)-Number(a.createdAt||0))).slice(0,12);if(eps.length){const title=document.createElement('div');title.className='memorySubTitle';title.textContent='我们的故事';box.appendChild(title);eps.forEach(e=>{const card=document.createElement('div');card.className='memoryEpisodeCard';card.innerHTML=`<div class="memoryEpisodeDate">${escapeHtml(e.date||'')}</div><div class="memoryEpisodeBody"><b>${escapeHtml(e.title||'共同经历')}</b><div>${escapeHtml(e.summary||'')}</div></div>`;box.appendChild(card)})}
}
