/* Iris v4.78 — vision transport rebuilt from scratch; text send path preserved. */
...
/* Iris v4.78 — vision transport is intentionally isolated from the normal chat pipeline. */
async function sendVisionMessage(c,text,attachments){
 ...
 const fail=msg=>{const detail=String(msg||"未知错误");console.error("Iris v4.78 vision",detail);showErr("图片发送失败："+detail);c.messages.push({role:"assistant",content:"图片发送失败：\n"+detail,timestamp:Date.now()});save();render()};
 ...
 try{
  const history=(c.messages||[]).slice(-11,-1).map(m=>{let v=m.content;if(Array.isArray(v))v=v.filter(x=>x?.type==="text").map(x=>x.text||"").join(" ");return {role:m.role,content:String(v||"").trim()}}).filter(m=>(m.role==="user"||m.role==="assistant")&&m.content);
...
async function send(){
 ...
 if(imageAttachments.length){
  ...
  }catch(e){console.error("Iris v4.78 image send",e);showErr("图片发送流程出错："+(e?.message||String(e)))}
  return;
 }
