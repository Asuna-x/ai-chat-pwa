/* Iris v4.82 — PWA image recognition: local image compression, Vision pass, then normal chat reply. */
...
async function sendVisionMessage(c,text,attachments){
 const visionEnabled=state.settings.visionEnabled===true;
 ...
 if(!visionEnabled){fail("视觉模型目前没有开启。请到 设置 → 视觉与图像 → 开启“启用视觉模型”。");return false}
 ...
 try{
  ...
  let vision,lastVisionError;let attempt=0;const maxAttempts=1;while(attempt<=maxAttempts){try{vision=await window.GChatAPI.visionChat({baseUrl:visionBase,apiKey:visionKey,model:visionModel,messages:[{role:"user",content}],temperature:.2,signal:controller.signal,max_tokens:1024});break}catch(err){lastVisionError=err;const msg=String(err?.message||err);if(!(/HTTP 429\b|code.?[:=]?\s*["\']?1305/i.test(msg))||attempt===maxAttempts)throw err;await new Promise(resolve=>setTimeout(resolve,5000));attempt++}}if(!vision)throw lastVisionError||new Error("视觉模型请求失败");const visualAnswer=String(vision.answer||"").trim();if(!visualAnswer)throw new Error("视觉模型返回为空");
